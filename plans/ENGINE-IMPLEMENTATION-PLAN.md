...
- audio subsystem
- animation subsystem
- spatial authoring and attachment-tuning subsystem (native schema/query/cook/rest-schematic, deterministic schema-v2 clip pose sampling, and exact Assets rest-rig workbench implemented; sampled attachment/IK/rendered review workflow deferred)
- physics subsystem
...
- Phase 5.73 has started with compatible v1 skeleton/clip loading, strict v2 skeleton/socket and v1 attachment-profile validation, deterministic schema-v2 clip pose sampling, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic spatial cooker, a deterministic rest-pose schematic evaluator, a revision-safe transient sessiond GET, exact baseline/candidate preview evaluations, a lease-gated semantic attachment operation, a strict sessiond-backed CLI adapter, a constrained Assets tuner with an exact native-evaluated rest-rig schematic, and lease-gated `sf-mcp` preview/review/apply/undo. Assets binds evidence to revision and operation identity, never moves cached geometry optimistically, and reconciles active operation events through App's one SSE subscription. Attachment evaluations remain unsampled, unpersisted, and not review evidence; generic bake/runtime consumption, sampled attachment integration, IK, prefab/item geometry, joint/clipping diagnostics, native camera/capture/review packets, and typed MCP validation/review resources remain ahead.
...
- Phase 5.73 now has a native schema/query/cook/rest-schematic foundation with isolated v2 skeleton/socket, schema-v2 sampled clip, and attachment fixtures, one explicit deterministic derived payload, a deterministic clip pose sampler, a read-only rest-pose attachment evaluator, a revision-safe transient sessiond GET, exact transient baseline/candidate evaluations in attachment preview, CLI and `sf-mcp` preview/approve/reject/apply/undo adapters, constrained shell tuning, and an accessible responsive Assets schematic with Front X/Y, Side Z/Y, and Top X/Z projections plus exact coordinates/diagnostics. GET requires exact bytes/revision and creates no journal state; preview binds validator-selected IDs and rechecks its lease after evaluation before creating the generic operation. The shell deep-validates bounded reports, fixes authored/candidate visuals to exact evidence identities, and uses one shared SSE subscription for guarded authoritative operation reconciliation plus captured-connection-only cleanup. The normal authored/runtime lane remains v1-compatible. The new sampler is not yet consumed by attachment evaluation, so rest evaluation still has `pose.sampled=false`, no item mesh/IK/joint/clipping/camera/capture evidence, and is not a review packet; prefab existence, joint/capsule parsing, generic bake/runtime consumption, sampled attachment integration, IK, rendered capture, review packets, operation-scoped validation/recapture, and typed MCP spatial resources remain deferred.
...
Status:
- native schema/query/cook/rest-schematic evaluation, deterministic schema-v2 clip pose sampling, semantic attachment operations, CLI adapter, and constrained Assets tuner with exact rest-rig schematic implemented; sampled attachment/IK/rendered capture/review workflow deferred
...
- `shader_forge_spatial validate|cook|evaluate-rest`, `engine build spatial`, the matching strict CLI commands, and `npm run test:spatial-tool` provide deterministic validation, one atomic complete derived socket/profile payload, and machine-readable rest-pose bone/socket/hand/item frames plus joint-segment endpoints over that same `AnimationSystem`
- `evaluate-rest` is a schematic query only: `pose.sampled=false`; item geometry plus IK, joint-limit, and clipping diagnostics are unavailable; authored pole world space is unresolved; and the output is not review evidence
- `GET /api/spatial/attachment/evaluate` now stages exact current authored animation bytes, binds the source to its validator-selected ID, and rechecks the requested revision without a lease or journal write
...
- `AnimationSystem` samples schema-v2 clip poses deterministically, but the spatial attachment evaluator does not consume them yet and does not evaluate IK, render attachments, or produce mesh/joint-limit/clipping diagnostics; its rest evaluator and the Assets schematic report those capabilities unavailable and do not claim camera/capture/review evidence
...
- generic bake/runtime consumption, sampled attachment integration/IK, prefab/item-mesh integration, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; native clip sampling, attachment preview/review/apply/undo, and the unsampled Assets schematic are implemented
...
9. audio system
10. animation system
11. spatial authoring and attachment tuning (native schema/query/cook/rest-schematic, deterministic clip sampling, and exact unsampled Assets workbench implemented; sampled attachment integration, IK, capture/review, and typed MCP widening continue before broad World/Assets visual polish)
