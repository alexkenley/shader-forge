...
- Godot `.tscn` conversion now maps node hierarchy, explicit position/rotation/scale, valid explicit perspective Camera3D optics, enabled CollisionShape3D/BoxShape3D geometry with disabled instances rejected, safe node-script ExtResource bindings, node-type-derived prefab tags, source comments, and mapped entity/component/script-binding counts; normalized node-name collisions are disambiguated deterministically
