...
- Phase 5.73 has started with compatible v1 skeleton loading, strict v2 skeleton/socket and v1 attachment-profile validation, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic spatial cooker, a lease-gated semantic attachment preview operation, and a strict sessiond-backed CLI adapter for preview/review/apply/undo. Generic bake/runtime consumption, sampling, IK, rendering/capture/review packets, shell, and MCP integration remain ahead.
...
- Phase 5.73 now has a native schema/query/cook foundation with isolated v2 skeleton/socket and attachment fixtures, one explicit deterministic derived payload, a sessiond attachment-preview operation over the generic journal, and CLI preview/approve/reject/apply/undo adapters. The normal authored/runtime lane remains v1-compatible, and prefab existence, joint/capsule parsing, generic bake/runtime consumption, sampling, IK, rendering/capture, review packets, operation-scoped validation/recapture, shell tuning, and MCP tools remain deferred.
