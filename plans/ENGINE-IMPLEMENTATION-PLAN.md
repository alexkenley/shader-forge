...
- audio subsystem
- animation subsystem
- spatial authoring and attachment-tuning subsystem (native schema/query/cook slice implemented; workflow deferred)
- physics subsystem
...
- [Engine Spatial Authoring Spec](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md): Phase 5.73 native schema/query/cook foundation and deferred spatial tuning workflow
...
- Phase 5.73 has started with compatible v1 skeleton loading, strict v2 skeleton/socket and v1 attachment-profile validation, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic spatial cooker, a lease-gated semantic attachment preview operation, a strict sessiond-backed CLI adapter, a constrained Assets tuner, and lease-gated `sf-mcp` preview/review/apply/undo. Generic bake/runtime consumption, sampling, IK, rendering/capture/review packets, and typed MCP validation/review resources remain ahead.
...
- Phase 5.73 now has a native schema/query/cook foundation with isolated v2 skeleton/socket and attachment fixtures, one explicit deterministic derived payload, a sessiond attachment-preview operation over the generic journal, CLI and `sf-mcp` preview/approve/reject/apply/undo adapters, and constrained shell tuning. The normal authored/runtime lane remains v1-compatible, and prefab existence, joint/capsule parsing, generic bake/runtime consumption, sampling, IK, rendering/capture, review packets, operation-scoped validation/recapture, and typed MCP spatial resources remain deferred.
...
9. audio system
10. animation system
11. spatial authoring and attachment tuning (native schema/query/cook slice started; workflow after the operation layer and before broad World/Assets visual polish)
...
- Phase 5.73 continuation: integrate the explicit spatial cooker with the generic bake/runtime lanes, then widen into sampling, IK, validation/recapture/review operations, and the shared tuning/capture workflow before World/Assets polish introduces attachment editing
