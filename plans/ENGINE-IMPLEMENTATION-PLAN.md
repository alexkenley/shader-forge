...
- Phase 5.73 has started with compatible v1 skeleton loading, strict v2 skeleton/socket and v1 attachment-profile validation, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic spatial cooker, a deterministic rest-pose schematic evaluator, a lease-gated semantic attachment preview operation, a strict sessiond-backed CLI adapter, a constrained Assets tuner, and lease-gated `sf-mcp` preview/review/apply/undo. The evaluator is unsampled and not review evidence; generic bake/runtime consumption, sampling, IK, item geometry, joint/clipping diagnostics, rendering/capture/review packets, and typed MCP validation/review resources remain ahead.
