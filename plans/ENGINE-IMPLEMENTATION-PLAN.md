...
Status:
- native schema/query/cook/rest-schematic evaluation, compatible v1/strict v2 profiles, deterministic schema-v2 clip sampling and sampled two-bone IK, authored v2 cone-twist/capsule parse/cook, end-to-end diagnose-only joint-limit and capsule/item surface-clearance diagnostics, deterministic depth-tested native close/player-camera sample capture, strict CLI queries, typed revision-bound MCP rest/sample reads, lease-free operation validation through thin CLI/MCP adapters, semantic attachment mutation revalidation, constrained Assets tuning, lease-gated sessiond recapture, atomic immutable review publication/reads, and thin CLI/MCP review adapters implemented; generic bake/runtime consumption, runtime collision/physics integration, and shell review presentation deferred
...
- Phase 5.73 continuation: add shell review presentation over the implemented native capture, sessiond immutable packets, CLI/MCP review adapters, and typed MCP resource without introducing a second attachment path
