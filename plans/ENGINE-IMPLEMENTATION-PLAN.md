...
- `ai/registry.json` now declares the first bounded read-only tools, object schemas, allowed clients, and composed skill; discovery plus exact allowlisted invocation/run routes and CLI adapters validate inputs, outputs, references, client narrowing, and ordered skill steps without arbitrary endpoint dispatch or mutation authority
