...
- `ai/registry.json` now declares the first bounded read-only tools, object schemas, allowed clients, and composed skill; `GET /api/ai/tools`, `GET /api/ai/skills`, and `engine ai tools|skills` expose discovery while strict validation rejects unknown tool references and any skill client widening
...
- additional hosted-provider execution, budget enforcement, spend accounting/pricing, and secure key storage beyond environment-backed OpenRouter BYOK plus its current per-request output ceiling and durable token-usage evidence
- tool/skill invocation, mutating permission enforcement through the existing operation/code-trust path, broader action-schema validation, and game-runtime exposure beyond the current read-only discovery registry
