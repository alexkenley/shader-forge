...
- Phase 5.73 has compatible v1 skeleton/clip/attachment loading, strict v2 skeleton/socket/attachment validation with item-space pole and semantic arm/palm capability, deterministic schema-v2 clip sampling and sampled two-bone secondary-hand IK, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic v1/v2 spatial cooker, strict native/`engine` CLI rest/sample queries, revision-safe transient sessiond rest/sample GETs with full animation-input manifests, exact rest baseline/candidate preview evaluations, a lease-gated semantic attachment operation, a constrained Assets tuner with exact native-evaluated rest/sampled rig schematics and resolved v2 pole projection, and lease-gated `sf-mcp` preview/review/apply/undo. Sampled IK reports physical reachability plus separate reach/contact/angular tolerance truth; v1 remains pre-IK and every sampled result remains non-review evidence. Assets consumes exact authored samples while candidate and MCP evidence remain rest-only. Generic bake/runtime consumption, sampled MCP evidence, prefab/item geometry, joint/clipping diagnostics, native camera/capture/review packets, and typed MCP validation/review resources remain ahead.
...
- Phase 5.73 now has a native schema/query/cook/rest-schematic foundation with isolated v2 skeleton/socket/attachment and sampled clip fixtures, deterministic v1/v2 derived payloads, deterministic sampled v2 two-bone IK, strict native/`engine` CLI rest/sample queries, revision-safe transient sessiond rest/sample GETs with complete source-revision binding, exact transient rest baseline/candidate evaluations in attachment preview, CLI and `sf-mcp` mutation adapters, constrained shell tuning, and accessible responsive Assets rest/sampled schematics with resolved v2 pole projection. Sampled evaluation requires an exact authored envelope time; v2 two-hand output applies primary attachment and secondary IK with numeric reach/contact/angular truth, while v1 stays pre-IK and one-hand marks IK not applicable. Assets consumes exact authored sampled evidence with full-manifest binding; candidate and MCP evidence remain rest-only. There is still no item mesh, available joint/clipping result, camera/capture evidence, or review packet; prefab existence, joint/capsule parsing, generic bake/runtime consumption, rendered capture, review packets, operation-scoped validation/recapture, and typed MCP spatial resources remain deferred.
...
- `evaluate-rest` is an unsolved schematic query only: `pose.sampled=false`; item geometry plus solved IK, joint-limit, and clipping diagnostics are unavailable; v1 pole world space is unresolved while v2 item-space poles resolve to world coordinates; and the output is not review evidence
- `GET /api/spatial/attachment/evaluate` and `/evaluate-sample` now stage exact current authored animation bytes, bind evaluator reports to the validator-selected ID/schema/skeleton/item/mode/perspective contract and requested phase/time, expose sorted `sourceRevisions`, and fail closed when any staged animation input changes, without a lease or journal write
...
- prefab existence plus joint-limit and diagnostic-capsule parsing are not part of the current loader slice
- `AnimationSystem` samples schema-v2 clip poses deterministically and the sampled spatial attachment evaluator applies v2 two-bone secondary-hand IK; the rest evaluator and candidate preview remain unsolved, while Assets displays exact authored solved samples. No current path renders attachments or produces item-mesh/available joint-limit/clipping diagnostics, camera/capture, or review evidence
...
- generic bake/runtime consumption, sampled MCP evidence, prefab/item-mesh integration, joint/capsule/clipping diagnostics, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; native clip sampling, v2 two-bone IK, transient sampled sessiond evidence, attachment preview/review/apply/undo, and exact non-review Assets rest/sampled schematics are implemented
...
- Phase 5.73 continuation: add prefab/item geometry, available joint/clipping diagnostics, validation/recapture/review operations, native cameras/capture, and immutable review packets on top of the implemented exact sampled Assets evidence; integrate the explicit cooker with generic bake/runtime and widen typed `sf-mcp` spatial reads/review only after those engine operations exist, without introducing a second attachment path
