...
- Phase 5.73 has compatible v1 skeleton/clip/attachment loading, strict v2 skeleton/socket/attachment validation, deterministic clip sampling and v2 two-bone IK, generation-safe query handles, an explicit atomic cooker, strict native/CLI rest/sample queries, deterministic native close-camera sample rendering, revision-safe sessiond evidence, semantic attachment operations, lease-free operation validation with thin CLI/MCP adapters, one scene/spatial apply dispatcher, a constrained Assets tuner, lease-gated `sf-mcp` mutation tools, read-only revision-bound `spatial_attachment_read`, and the durable bounded validation-summary/event journal. Rest/sample resolve independent render-procgeo visual evidence and authored prefab collision through `DataFoundation`, then emit diagnose-only rest-relative cone-twist and capsule-axis-to-oriented-box surface-clearance diagnostics without mutating or clamping the pose. `capture-sample` depth-tests posed bone prisms and the authored procgeo box into four clean deterministic PNGs with explicit camera/bounds/lighting metadata; it does not publish review evidence. Sessiond validates private proposed bytes in a complete staged snapshot, reduces at most 64 requested samples to controlled counts, rejects manifest/CAS drift, and independently revalidates apply/undo while the covering write lease is held. Missing authored inputs remain exact unavailable evidence, tangency is clear, and the render box never substitutes for collision. Generic bake/runtime consumption, runtime collision/physics integration, authored player-camera capture, immutable review packets, recapture, and typed review adapters remain ahead.
...
- Phase 5.73 now has native schema/query/cook/rest/sample foundations, strict authored cone-twist/capsule skeleton metadata, strict typed prefab box-collision source truth, deterministic v2 two-bone IK, native numeric joint-limit and capsule/item surface-clearance diagnostics, deterministic native close-camera sample rendering, fail-closed sessiond/Assets validation, revision-safe transient evidence, exact baseline/candidate preview, lease-free operation validation through CLI and `sf-mcp`, native apply/undo revalidation through one semantic dispatcher, CLI and `sf-mcp` mutation adapters, read-only revision-bound `spatial_attachment_read`, constrained shell tuning, responsive Assets schematics, and a strict restart-safe bounded validation journal. Authored runtime cameras exist, but player-camera capture, runtime physics collision results, revision-bound review publication, immutable review packets, recapture, and typed review adapters remain deferred.
...
Status:
- native schema/query/cook/rest-schematic evaluation, compatible v1/strict v2 profiles, deterministic schema-v2 clip sampling and sampled two-bone IK, authored v2 cone-twist/capsule parse/cook, end-to-end diagnose-only joint-limit and capsule/item surface-clearance diagnostics, deterministic depth-tested native close-camera sample capture, strict CLI queries, typed revision-bound MCP rest/sample reads, lease-free operation validation through thin CLI/MCP adapters, semantic attachment mutation revalidation, and constrained Assets tuning implemented; generic bake/runtime consumption, runtime collision/physics integration, authored player-camera capture, typed review adapters, and immutable review publication deferred
...
- generic bake/runtime consumption, runtime collision/physics integration, sessiond recapture/review packets, authored player-camera capture, rendered spatial tuning, and typed review adapters are still deferred; authored joint/capsule schema and cooking, native numeric joint-limit and capsule/item surface-clearance diagnostics through CLI/sessiond/Assets, strict prefab collision source truth, native clip sampling, v2 two-bone IK, deterministic native close-camera PNG capture, transient sessiond evidence, lease-free operation validation through CLI/`sf-mcp`, native mutation revalidation, read-only revision-bound `spatial_attachment_read`, attachment preview/review/apply/undo, and exact non-review Assets rest/sampled schematics are implemented
...
- Phase 5.73 continuation: add recapture/review operations, revision-bound frame capture, immutable review packets, and the corresponding typed review tools without introducing a second attachment path
