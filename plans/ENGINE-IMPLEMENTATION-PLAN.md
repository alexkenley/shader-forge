...
- `AnimationSystem` does not own prefab lookup; evaluator-side prefab/render-procgeo resolution now uses `DataFoundation`, while joint-limit, diagnostic-capsule, and collision parsing remain open
...
- generic bake/runtime consumption, sampled MCP evidence, explicit item collision geometry, joint/capsule/clipping diagnostics, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; prefab-bound authored visual-box evidence, native clip sampling, v2 two-bone IK, transient sessiond evidence, attachment preview/review/apply/undo, and exact non-review Assets rest/sampled schematics are implemented
...
- Phase 5.73 continuation: add explicit item collision geometry, skeleton capsules, joint limits, and available clipping diagnostics on top of the implemented prefab-bound visual geometry; then add validation/recapture/review operations, native cameras/capture, and immutable review packets before widening typed `sf-mcp` spatial reads/review, without introducing a second attachment path
