...
- audio subsystem
- animation subsystem
- spatial authoring and attachment-tuning subsystem (native schema/query/cook/rest-schematic, compatible v1/strict v2 attachment profiles, deterministic clip sampling and sampled v2 two-bone IK, lease-free operation validation, semantic mutation revalidation, strict CLI queries, and exact Assets rest/sampled rig workbench implemented; rendered review workflow deferred)
- physics subsystem
...
- [Engine Spatial Authoring Spec](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md): Phase 5.73 native schema/query/cook/rest-schematic foundation, operation-scoped validation, exact Assets rest/sampled rig workbench, and immutable rendered review workflow
...
- [Engine Code Trust Spec](../docs/specs/ENGINE-CODE-TRUST-SPEC.md): Phase 5.95
...
- Phase 5.73 has compatible v1 skeleton/clip/attachment loading, strict v2 skeleton/socket/attachment validation, deterministic clip sampling and v2 two-bone IK, generation-safe query handles, an explicit atomic cooker, strict native/CLI rest/sample queries, deterministic native close/player-camera sample rendering, revision-safe sessiond evidence, semantic attachment operations, lease-free operation validation with thin CLI/MCP adapters, one scene/spatial apply dispatcher, a constrained Assets tuner, lease-gated `sf-mcp` mutation tools, read-only revision-bound `spatial_attachment_read`, exact lease-gated recapture, atomic immutable review packets and reads, thin CLI/MCP review adapters, Assets packet publication/loading/presentation, and the durable bounded validation-summary/event journal. Rest/sample resolve independent render-procgeo visual evidence and authored prefab collision through `DataFoundation`, then emit diagnose-only rest-relative cone-twist and capsule-axis-to-oriented-box surface-clearance diagnostics without mutating or clamping the pose. `capture-sample` depth-tests posed bone prisms and the authored procgeo box into four bounded close views plus an optional authored player-camera view with explicit source/camera/bounds/lighting metadata. Sessiond validates private proposed bytes, expands authored phase samples, enforces read/capture/review leases, rejects drift, atomically publishes a new packet, and independently revalidates apply/undo. Assets displays only packet-referenced PNGs as review evidence. Missing authored inputs remain exact unavailable evidence, tangency is clear, and the render box never substitutes for collision. Generic bake/runtime consumption and runtime collision/physics integration remain ahead.
...
- Phase 5.75 has now started through a shell-side level-authoring slice with repo-backed scene/prefab round-trip, placed-entity hierarchy plus transform editing, first prefab component payload editing, draft-preserving `Edit`/`Verify`, detached dirty drafts across incompatible project/session changes, one-click save/build/play, outliner/details/assets surfaces, and revision-bound semantic scene-asset operations with exact leases, native validation, and a thin `sf-mcp` adapter.
- Phase 5.9 has now started through source-controlled AI provider manifests, a shared provider inspection/smoke-test core, deterministic fake-provider coverage, optional local Ollama, fixed-endpoint OpenRouter/Kimi K3 and GLM 5.2 BYOK request paths, bounded transient retry with explicit fallback, shell/sessiond/CLI AI inspection surfaces, a bounded process-scoped sessiond job queue with cancellation/status/events, bounded metadata-only terminal history, durable per-workspace provider token-usage evidence, a durable opt-in monthly queued-request admission cap, a bounded schema-validated read-only tool/skill registry and execution path, and a dedicated deterministic scaffold harness.
...
- Phase 6.2 has now started through source-controlled Debug/Release desktop presets, release-layout inspection plus packaging commands, packaged authored runtime roots alongside bundled cooked outputs, launch scripts, a real ZIP hook, shell/sessiond packaging surfaces, and deterministic packaging coverage.
...
- The shell `World` workspace now loads revision-bearing scene and prefab assets from the active session, supports deterministic semantic save/reload/create/duplicate flows, and exposes the first real outliner/details/assets authoring lane with placed-entity create/duplicate/delete, transform editing, and prefab component payload editing. `Verify` is read-only without discarding drafts; incompatible workspace changes detach dirty drafts; Play saves through exact lease-gated operations before requesting Build + Run and leaves actionable failures visible.
...
- Phase 5.8 now maps active Unity text-YAML `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs with source file IDs and local transforms. `m_IsActive: 0` objects and their hierarchy subtrees are omitted. Enabled valid perspective `Camera` optics and default-layer non-trigger `BoxCollider` center/size geometry without PhysicMaterial references reuse strict prefab components while disabled, trigger, non-default-layer, and material-bearing components fail closed, and `MonoBehaviour` GUIDs resolve through `.cs.meta` into binding-specific script-porting provenance; prefab instances, other components, disabled camera/collider and special collider preservation, orthographic cameras, assets, script behavior/fields, runtime physics consumption, and coordinate-system remediation remain manual.
...
- Phase 5.9 has now started through `ai/providers.toml`, `ai/registry.json`, shared provider execution and registry inspection code under `tools/shared/`, optional Ollama plus OpenRouter/Kimi K3 and GLM 5.2 execution, `engine_sessiond` AI provider/job/history/usage/tool/skill routes, matching `engine ai` commands, a shell-side workspace AI status card, and `test:ai-scaffold`.
- Phase 5.95 has now started through `tooling/policy/code-access-policy.json`, shared `engine_sessiond`/CLI code-trust evaluation, tracked artifact metadata under `.shader-forge/code-trust-artifacts.json`, hash-backed artifact verification, explicit promote/quarantine tooling, queued approval handling for review-gated assistant operations, shell-side workspace approval controls, and `test:code-trust-scaffold`.
...
- Phase 5.8 groundwork now exists through fixture-backed Shader Forge project skeleton conversion, exact startup-setting translation/provenance, deterministic duplicate-scene disambiguation, Unity and Godot text-scene hierarchy/local-transform mapping plus perspective camera, box-collision, and exact script-binding provenance, fail-closed unresolved startup handling, deterministic no-declaration fallback, and production-bake validation; full asset/material import, behavior translation, broader component/prefab-instance mapping, conflict-safe reimport, exporter-assisted Unreal actor extraction, and shell-side migration inspection still remain
...
- Phase 5.73 now has native schema/query/cook/rest/sample foundations, strict authored cone-twist/capsule skeleton metadata, strict typed prefab box-collision source truth, deterministic v2 two-bone IK, native numeric joint-limit and capsule/item surface-clearance diagnostics, deterministic native close/player-camera sample rendering, fail-closed sessiond/Assets validation, revision-safe transient evidence, exact baseline/candidate preview, lease-free operation validation through CLI and `sf-mcp`, native apply/undo revalidation through one semantic dispatcher, CLI and `sf-mcp` mutation adapters, read-only revision-bound `spatial_attachment_read`, constrained shell tuning, responsive Assets schematics, exact lease-gated sessiond recapture, atomic immutable review packets and reads, thin CLI/MCP review adapters, Assets packet publication/loading/presentation, and a strict restart-safe bounded validation journal. Runtime physics collision results remain deferred.
...
- Phase 5.75 groundwork now exists through shell-side revision-bearing scene/prefab round-trip, lease-gated semantic save/create/duplicate with native `DataFoundation` validation and authoritative reconciliation, placed-entity hierarchy plus transform editing, first prefab component payload editing, local undo/redo, asset reassignment, draft-preserving `Edit`/`Verify`, detached dirty workspace drafts, guarded destructive navigation, and save-then-Build + Run Play, but multi-file rename, transform gizmos, broader scene/component authoring, and procedural bake-back flows still remain
- Phase 5.9 groundwork now exists through source-controlled provider manifests, shared provider inspection plus smoke-test execution, deterministic fake coverage, optional Ollama readiness, credential-safe OpenRouter/Kimi K3 and GLM 5.2 BYOK lanes, bounded transient retry with explicit fallback, shell/sessiond/CLI surfaces, a bounded process-scoped sessiond job queue with cancellation/status/events, bounded metadata-only terminal history, durable per-workspace provider token-usage evidence, a durable opt-in monthly queued-request admission cap, and schema-validated read-only tool/skill execution that now composes the existing live runtime/profile service, but resumable jobs, additional hosted adapters, token/spend budgets and pricing, mutating tool/skill permission enforcement, and gameplay-facing/native-assistant integration still remain
...
- Phase 6.2 groundwork now exists through source-controlled Debug/Release desktop presets, release-layout inspection plus packaging commands, packaged authored runtime roots plus bundled cooked outputs, generated launch scripts, a real ZIP hook, and shell/sessiond package surfaces, but runtime-build orchestration, installers, signing, platform-specific presets, additional hooks, and real cooked-runtime loading still remain
...
Status:
- native schema/query/cook/rest-schematic evaluation, compatible v1/strict v2 profiles, deterministic schema-v2 clip sampling and sampled two-bone IK, authored v2 cone-twist/capsule parse/cook, end-to-end diagnose-only joint-limit and capsule/item surface-clearance diagnostics, deterministic depth-tested native close/player-camera sample capture, strict CLI queries, typed revision-bound MCP rest/sample reads, lease-free operation validation through thin CLI/MCP adapters, semantic attachment mutation revalidation, constrained Assets tuning, lease-gated sessiond recapture, atomic immutable review publication/reads, thin CLI/MCP review adapters, and Assets packet presentation implemented; generic bake/runtime consumption and runtime collision/physics integration deferred
- canonical contract: [ENGINE-SPATIAL-AUTHORING-SPEC.md](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md)
- v1-compatible skeleton/attachment loading, strict v2 skeleton/socket/attachment parsing with item-space pole and arm/palm capability checks, and generation-safe query handles now exist in `AnimationSystem`
...
- `shader_forge_spatial validate|cook|evaluate-rest|evaluate-sample`, `engine build spatial`, the matching strict CLI commands, and `npm run test:spatial-tool` provide deterministic validation, versioned atomic derived socket/profile payloads, rest geometry, v1 pre-IK compatibility, and v2 sampled two-bone IK over exact authored envelope phase/time through the same `AnimationSystem`
- `evaluate-rest` is an unsolved schematic query only: `pose.sampled=false`; it resolves independent render-procgeo visual corners and authored prefab collision through `DataFoundation`, then emits exact typed rest-relative joint-limit and capsule-axis-to-oriented-box surface-clearance diagnostics without mutating or clamping the pose; solved IK remains unavailable and the output is diagnostic schematic evidence, not runtime collision or review evidence
- `GET /api/spatial/attachment/evaluate` and `/evaluate-sample` now stage exact current animation bytes, every authored content TOML, and the data-foundation manifest; bind evaluator reports to the selected profile and requested sample; expose sorted `sourceRevisions`; reject symbolic paths; and fail closed when any staged input changes, without a lease or journal write
...
- `engine spatial preview|validate-operation|approve|reject|apply|undo` now adapts that same sessiond contract with strict arguments, fatal UTF-8 candidate/sample input, validation without credentials, environment-only mutation credentials, and no implicit coordination or write bypass
...
- `AnimationSystem` samples schema-v2 clip poses deterministically and the sampled evaluator applies v2 two-bone IK before joint-limit and clipping evaluation. `shader_forge_spatial capture-sample` renders that exact pose and authored procgeo box through a deterministic depth-tested native triangle path into four clean close-camera PNGs. Assets still displays exact authored samples as independent visual/collision boxes, capsule segments, diagnose-only joint-limit `PASS`/`FAIL`, and capsule/item `CLEAR`/`OVERLAP`; it does not promote diagnostics or standalone native frames into runtime collision or review evidence
...
- generic bake/runtime consumption and runtime collision/physics integration are still deferred; authored joint/capsule schema and cooking, native numeric joint-limit and capsule/item surface-clearance diagnostics through CLI/sessiond/Assets, strict prefab collision source truth, native clip sampling, v2 two-bone IK, deterministic native close/player-camera PNG capture, transient sessiond evidence, lease-free operation validation through CLI/`sf-mcp`, native mutation revalidation, read-only revision-bound `spatial_attachment_read`, attachment preview/review/apply/undo, exact sessiond recapture/immutable packet reads, thin CLI/MCP review adapters, non-review Assets rest/sampled schematics, and packet-referenced Assets review presentation are implemented
...
- `sf-mcp` now exposes strict whole scene/prefab save, create, and duplicate preview plus shared approve/apply/undo over the same exact leases and native semantic validator used by World
...
- Unity text-YAML `.unity` conversion now maps active `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs, preserves source file IDs, converts local quaternions to deterministic Euler degrees, omits `m_IsActive: 0` hierarchy subtrees, maps enabled valid perspective Camera optics and default-layer non-trigger/no-PhysicMaterial BoxCollider geometry while rejecting unsupported collider semantics, resolves MonoBehaviour GUID bindings through `.cs.meta`, and reports each mapped coverage class separately
- Godot `.tscn` conversion now maps node hierarchy, explicit position/rotation/scale, valid explicit perspective Camera3D optics, enabled non-Area3D CollisionShape3D/BoxShape3D geometry with disabled and Area3D trigger instances rejected, safe node-script ExtResource bindings, node-type-derived prefab tags, source comments, and mapped entity/component/script-binding counts; normalized node-name collisions are disambiguated deterministically
...
- Unity prefab-instance/remaining-component/camera-collider-semantic/coordinate-system conversion, Unreal scene hierarchy and transform extraction, plus Godot transform matrices, resource instances, other components, script behavior/fields, camera enabled/current semantics, and physics-body, disabled-collider-preservation, layer, and material semantics
...
Current checkpoint now implemented:
- `ai/providers.toml` now defines deterministic `fake`, optional `ollama`, disabled-by-default OpenRouter/Kimi K3 and GLM 5.2 BYOK providers, and reserved hosted-provider entries for later adapters
...
- `[request]` policy now bounds retries to zero through two and explicit fallbacks to at most two configured provider IDs; only transient network failures and HTTP 408/425/429/500/502/503/504 responses use the 100 ms retry/fallback path, while cancellation, configuration/authentication failures, other 4xx responses, and malformed/empty/oversized responses fail immediately
...
- terminal jobs now atomically persist to a bounded 128-record workspace history containing lifecycle/provider/token metadata only; `GET /api/ai/history` and `engine ai history` expose status/limit-filtered reads without prompts, responses, raw errors, or credentials
- successful queued calls with provider-reported usage now atomically persist cumulative per-workspace and per-provider token counts under `.shader-forge/ai-usage.json`; `GET /api/ai/usage` and `engine ai usage` expose the credential-free summary
- `[request].monthly_request_limit` now provides an opt-in durable UTC-month admission cap for queued non-fake jobs under `.shader-forge/ai-budget.json`; admission occurs before provider execution, is not refunded, covers retries/fallbacks as one job, and is exposed through `GET /api/ai/budget` plus `engine ai budgets`
- `ai/registry.json` now declares bounded read-only tools, object schemas, allowed clients, and composed skills; exact allowlisted invocation/run routes and CLI adapters validate inputs, outputs, references, client narrowing, and ordered steps, while the workspace-health skill reuses the existing live runtime/profile service without arbitrary endpoint dispatch or mutation authority
...
- typed external-agent coverage now includes the existing lease-gated whole scene/prefab operation rather than a generic file-write bypass
...
Current checkpoint now implemented:
- source-controlled default policy data now exists under `tooling/policy/code-access-policy.json`
- a shared policy core now exists under `tools/shared/code-trust-policy.mjs` so sessiond, CLI, and future assistant surfaces do not invent separate trust rules
- `engine_sessiond` now exposes `GET /api/code-trust/summary` plus `POST /api/code-trust/evaluate`, and policy-relevant file writes/build/runtime start requests now pass through the same local policy layer
- `engine_sessiond` now also exposes approval listing and approval-decision routes so `review_required` transitions can be queued and replayed after explicit human approval
- `engine policy inspect`, `engine policy check`, `engine policy approvals`, `engine policy approve`, and `engine policy deny` now expose the same trust boundary from the terminal without requiring ad hoc HTTP calls
...
Still ahead inside this phase:
- signed artifact verification and stronger plugin package verification
- explicit shader/code hot-reload upgrade contracts after verification, instead of the current restart-only lane
- native tooling surfaces beyond the current shell-side summary card and workspace approval list
...
- the default preset's `archive_zip` hook now emits a standard ZIP beside the release layout and reports the exact output path; the dependency-free writer fails explicitly beyond ZIP32 limits
...
- platform-specific preset families beyond the current Debug/Release desktop pair
- installer generation, signing hooks, and additional platform-hook execution beyond the built-in ZIP hook
...
9. audio system
10. animation system
11. spatial authoring and attachment tuning (native schema/query/cook, deterministic rest/sample evaluation with v2 two-bone IK plus joint-limit and capsule/item surface-clearance diagnostics, operation validation through CLI/MCP, revision-bound typed MCP rest/sample reads, and the exact Assets workbench implemented; frame capture and review packets continue before broad World/Assets visual polish)
...
- Phase 5.73 continuation completed: Assets presents packet-referenced immutable reviews over the implemented native capture, sessiond packet publisher, CLI/MCP adapters, and typed MCP resource without a second attachment path
...
- Phase 5.9 continuation: widen the provider/status/test foundation beyond the first OpenRouter adapter into queued requests, additional hosted providers, budgets, and shared tool/skill registries while staying behind the Phase 5.95 trust boundary for risky assistant actions
...
- Phase 6.2 continuation: widen the export/package foundation beyond its desktop Debug/Release layouts and ZIPs into runtime-build orchestration, platform presets, installers/signing, and a real cooked-runtime release flow
