...
- Godot `.tscn` conversion now maps node hierarchy, explicit position/rotation/scale, valid explicit perspective Camera3D optics, CollisionShape3D/BoxShape3D geometry, safe node-script ExtResource bindings, node-type-derived prefab tags, source comments, and mapped entity/component/script-binding counts; normalized node-name collisions are disambiguated deterministically
...
- Unity prefab-instance/remaining-component/camera-collider-semantic/coordinate-system conversion, Unreal scene hierarchy and transform extraction, plus Godot transform matrices, resource instances, other components, script behavior/fields, camera enabled/current semantics, and physics-body/disabled/layer/material semantics
