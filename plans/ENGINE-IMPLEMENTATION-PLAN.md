...
- Phase 5.8 groundwork now exists through fixture-backed Shader Forge project skeleton conversion, exact startup-setting translation/provenance, deterministic duplicate-scene disambiguation, Unity and Godot text-scene hierarchy/local-transform mapping plus perspective camera and box-collision geometry, fail-closed unresolved startup handling, deterministic no-declaration fallback, and production-bake validation; full asset/material import, broader component/prefab-instance mapping, conflict-safe reimport, exporter-assisted Unreal actor extraction, and shell-side migration inspection still remain
...
- Godot `.tscn` conversion now maps node hierarchy, explicit position/rotation/scale, valid explicit perspective Camera3D optics, CollisionShape3D/BoxShape3D geometry, node-type-derived prefab tags, source comments, and mapped entity/component counts; normalized node-name collisions are disambiguated deterministically
...
- Unity prefab-instance/remaining-component/camera-collider-semantic/coordinate-system conversion, Unreal scene hierarchy and transform extraction, plus Godot transform matrices, resource instances, other components, camera enabled/current semantics, and physics-body/disabled/layer/material semantics
