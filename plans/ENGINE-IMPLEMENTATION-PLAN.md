...
- Phase 5.73 now has native schema/query/cook/rest/sample foundations, strict authored cone-twist/capsule skeleton metadata, strict typed prefab box-collision source truth, deterministic v2 two-bone IK, native numeric joint-limit and capsule/item surface-clearance diagnostics, fail-closed sessiond/Assets validation, revision-safe transient evidence, exact baseline/candidate preview, lease-free operation validation, native apply/undo revalidation through one semantic dispatcher, CLI and `sf-mcp` mutation adapters, read-only revision-bound `spatial_attachment_read`, constrained shell tuning, responsive Assets schematics, and a strict restart-safe bounded validation journal. Sessiond verifies private proposed hashes and full staged manifests, evaluates rest plus requested samples sequentially, rejects drift/stale CAS, and exposes no raw native diagnostics. Authored runtime cameras now exist, but rendered attachments, runtime physics collision results, revision-bound capture evidence, immutable review packets, recapture, and thin CLI/MCP validation adapters remain deferred.
...
Status:
- native schema/query/cook/rest-schematic evaluation, compatible v1/strict v2 profiles, deterministic schema-v2 clip sampling and sampled two-bone IK, authored v2 cone-twist/capsule parse/cook, end-to-end diagnose-only joint-limit and capsule/item surface-clearance diagnostics, strict CLI queries, typed revision-bound MCP rest/sample reads, lease-free operation validation, semantic attachment mutation revalidation, and constrained Assets tuning implemented; generic bake/runtime consumption, runtime collision/physics integration, typed CLI/MCP validation/review adapters, and rendered capture/review workflow deferred
...
- generic bake/runtime consumption, runtime collision/physics integration, recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and typed CLI/`sf-mcp` validation/review adapters are still deferred; authored joint/capsule schema and cooking, native numeric joint-limit and capsule/item surface-clearance diagnostics through CLI/sessiond/Assets, strict prefab collision source truth, native clip sampling, v2 two-bone IK, transient sessiond evidence, lease-free operation validation, native mutation revalidation, read-only revision-bound `spatial_attachment_read`, attachment preview/review/apply/undo, and exact non-review Assets rest/sampled schematics are implemented
...
9. audio system
10. animation system
11. spatial authoring and attachment tuning (native schema/query/cook, deterministic rest/sample evaluation with v2 two-bone IK plus joint-limit and capsule/item surface-clearance diagnostics, strict CLI queries, revision-bound typed MCP rest/sample reads, and the exact Assets workbench implemented; frame capture, review packets, and typed validation/review widening continue before broad World/Assets visual polish)
...
- Phase 5.73 continuation: expose the implemented sessiond validation route through thin CLI/MCP adapters, then add recapture/review operations, revision-bound frame capture, immutable review packets, and the corresponding typed review tools without introducing a second attachment path
