...
- Phase 5.8 now maps Unity text-YAML `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs with source file IDs and local transforms; prefab instances, components, assets, and coordinate-system remediation remain manual.
...
- Unity text-YAML `.unity` conversion now maps `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs, preserves source file IDs, converts local quaternions to deterministic Euler degrees, and reports mapped scene entities
...
- Unity prefab-instance/component/coordinate-system conversion, Unreal scene hierarchy and transform extraction, plus Godot transform matrices, resource instances, and component payloads
