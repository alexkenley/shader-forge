...
- Phase 5.95 has now started through a first code-trust slice with source-controlled policy data, shared sessiond/CLI policy evaluation, tracked assistant/code-path artifact metadata, explicit assistant-triggered compile/load/apply gating, and deterministic hot-reload policy coverage.
...
- Phase 5.95 has now started through `tooling/policy/code-access-policy.json`, shared `engine_sessiond`/CLI code-trust evaluation, tracked artifact metadata under `.shader-forge/code-trust-artifacts.json`, assistant-triggered build/runtime load/apply gating, shell-side workspace inspection, and `test:code-trust-scaffold`.
...
- Phase 5.95 groundwork now exists through source-controlled code-trust policy data, shared sessiond/CLI policy evaluation, explicit assistant-triggered compile/load/apply gating, tracked artifact-origin metadata, and deterministic policy/hot-reload coverage, but there is still no approval queue, signed-artifact verification, plugin-install verification, trust-promotion workflow, or real code hot reload yet
...
- `engine policy inspect` and `engine policy check` now expose the same trust boundary from the terminal without requiring a live backend
...
- the shell `Workspace` panel now exposes the active policy source, supported authored hot-reload roots, and recent tracked artifacts
- deterministic coverage now exists through `scripts/test-engine-code-trust-scaffold.mjs` for allowed authored-content hot reload plus rejected assistant-triggered engine apply/compile/load transitions

Still ahead inside this phase:
- a persistent review and approval workflow for `review_required` decisions instead of surfacing diagnostics alone
- trusted-artifact hashing/signing and stronger plugin package verification
- trust-promotion and quarantine flows beyond direct policy edits
- explicit shader/code hot-reload upgrade contracts after verification, instead of the current restart-only lane
- native tooling surfaces beyond the current shell-side summary card
