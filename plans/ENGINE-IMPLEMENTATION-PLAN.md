...
- Phase 5.8 now maps Unity text-YAML `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs with source file IDs and local transforms. Valid perspective `Camera` optics and `BoxCollider` center/size geometry reuse strict prefab components, while `MonoBehaviour` GUIDs resolve through `.cs.meta` into binding-specific script-porting provenance; prefab instances, other components, camera/collider enabled state, orthographic cameras, trigger/layer/material collider semantics, assets, script behavior/fields, runtime physics consumption, and coordinate-system remediation remain manual.
