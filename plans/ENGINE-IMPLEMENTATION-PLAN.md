...
- Phase 5.8 now maps Unity text-YAML `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs with source file IDs and local transforms. Valid perspective `Camera` optics and `BoxCollider` center/size geometry reuse strict prefab components; prefab instances, other components, camera/collider enabled state, orthographic cameras, trigger/layer/material collider semantics, assets, runtime physics consumption, and coordinate-system remediation remain manual.
...
- Unity text-YAML `.unity` conversion now maps `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs, preserves source file IDs, converts local quaternions to deterministic Euler degrees, maps valid perspective Camera optics and BoxCollider geometry, and reports mapped scene entities
...
- Unity prefab-instance/remaining-component/camera-collider-semantic/coordinate-system conversion, Unreal scene hierarchy and transform extraction, plus Godot transform matrices, resource instances, and component payloads
