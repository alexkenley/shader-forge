...
- Phase 5.9 has now started through `ai/providers.toml`, shared provider inspection/smoke-test code under `tools/shared/engine-ai-service.mjs`, optional Ollama plus OpenRouter/Kimi K3 and GLM 5.2 execution, `engine_sessiond` AI inspection/test routes, `engine ai providers|test|request`, a shell-side workspace AI status card, and `test:ai-scaffold`.
...
- Phase 5.9 groundwork now exists through source-controlled provider manifests, shared provider inspection plus smoke-test execution, deterministic fake coverage, optional Ollama readiness, credential-safe OpenRouter/Kimi K3 and GLM 5.2 BYOK lanes, and shell/sessiond/CLI surfaces, but queued request lifecycle, additional hosted adapters, budgets, tool/skill registries, and gameplay-facing/native-assistant integration still remain
