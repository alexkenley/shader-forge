...
- audio subsystem
- animation subsystem
- spatial authoring and attachment-tuning subsystem (native schema/query/cook/rest-schematic, deterministic schema-v2 clip and sampled attachment evaluation with pre-IK two-hand output, strict CLI queries, and exact Assets rest-rig workbench implemented; IK/sampled shell evidence/rendered review workflow deferred)
- physics subsystem
...
- Phase 5.73 has started with compatible v1 skeleton/clip loading, strict v2 skeleton/socket and v1 attachment-profile validation, deterministic schema-v2 clip and sampled attachment evaluation with pre-IK two-hand output, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic spatial cooker, strict native/`engine` CLI rest/sample queries, a revision-safe transient rest-only sessiond GET, exact rest baseline/candidate preview evaluations, a lease-gated semantic attachment operation, a constrained Assets tuner with an exact native-evaluated rest-rig schematic, and lease-gated `sf-mcp` preview/review/apply/undo. Assets binds evidence to revision and operation identity, never moves cached geometry optimistically, and reconciles active operation events through App's one SSE subscription. Native sampled evaluation is not review evidence; two-hand output remains pre-IK, while one-hand output reports secondary IK not applicable. Sessiond, Assets, and MCP remain rest-only. Generic bake/runtime consumption, IK, sampled sessiond/shell evidence, prefab/item geometry, joint/clipping diagnostics, native camera/capture/review packets, and typed MCP validation/review resources remain ahead.
...
- Phase 5.73 now has a native schema/query/cook/rest-schematic foundation with isolated v2 skeleton/socket, schema-v2 sampled clip, and attachment fixtures, one explicit deterministic derived payload, deterministic clip and sampled attachment evaluation with pre-IK two-hand output, strict native/`engine` CLI rest/sample queries, a revision-safe rest-only sessiond GET, exact transient rest baseline/candidate evaluations in attachment preview, CLI and `sf-mcp` mutation adapters, constrained shell tuning, and an accessible responsive Assets rest schematic. Sampled evaluation requires an exact authored envelope time and applies primary attachment; two-hand results report unavailable secondary IK, while one-hand results report it not applicable. The shell/sessiond/MCP evidence lane remains rest-only, with no item mesh/IK/joint/clipping/camera/capture evidence and no review packet; prefab existence, joint/capsule parsing, generic bake/runtime consumption, IK, sampled operation evidence, rendered capture, review packets, operation-scoped validation/recapture, and typed MCP spatial resources remain deferred.
...
Status:
- native schema/query/cook/rest-schematic evaluation, deterministic schema-v2 clip and sampled attachment evaluation with pre-IK two-hand output, strict CLI queries, semantic attachment operations, and constrained Assets tuner with exact rest-rig schematic implemented; IK/sampled shell evidence/rendered capture/review workflow deferred
...
- generic bake/runtime consumption, secondary-hand IK, sampled sessiond/shell evidence, prefab/item-mesh integration, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; native clip/pre-IK attachment sampling, attachment preview/review/apply/undo, and the unsampled Assets schematic are implemented
...
9. audio system
10. animation system
11. spatial authoring and attachment tuning (native schema/query/cook/rest-schematic, deterministic clip and sampled attachment evaluation with pre-IK two-hand output, strict CLI queries, and exact unsampled Assets workbench implemented; IK, sampled shell evidence, capture/review, and typed MCP widening continue before broad World/Assets visual polish)
...
- Phase 5.73 continuation: add secondary-hand IK to the native sampled attachment evaluator, then add sampled sessiond/shell evidence, prefab/item geometry, available joint/clipping diagnostics, validation/recapture/review operations, native cameras/capture, and immutable review packets; integrate the explicit cooker with generic bake/runtime and widen typed `sf-mcp` spatial reads/review only after those engine operations exist, without introducing a second attachment path
