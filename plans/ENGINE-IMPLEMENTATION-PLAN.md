...
- Phase 5.73 has started with compatible v1 skeleton loading, strict v2 skeleton/socket and v1 attachment-profile validation, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic spatial cooker, and a lease-gated semantic attachment preview operation. Generic bake/runtime consumption, sampling, IK, rendering/capture/review, shell, CLI preview, and MCP integration remain ahead.
...
- Phase 5.73 now has a native schema/query/cook foundation with isolated v2 skeleton/socket and attachment fixtures, one explicit deterministic derived payload, and a sessiond attachment-preview operation over the generic journal. The normal authored/runtime lane remains v1-compatible, and prefab existence, joint/capsule parsing, generic bake/runtime consumption, sampling, IK, rendering/capture, review packets, remaining operations, shell tuning, CLI preview, and MCP tools remain deferred.
...
- generic bake/runtime consumption, sampling/IK, validate/recapture/review-packet operations, screenshot capture, shell tuning, and `sf-mcp` spatial tools are still deferred
