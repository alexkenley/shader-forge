...
- Phase 5.8 now maps Unity text-YAML `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs with source file IDs and local transforms. Enabled valid perspective `Camera` optics and `BoxCollider` center/size geometry reuse strict prefab components while disabled instances fail closed, and `MonoBehaviour` GUIDs resolve through `.cs.meta` into binding-specific script-porting provenance; prefab instances, other components, disabled camera/collider preservation, orthographic cameras, trigger/layer/material collider semantics, assets, script behavior/fields, runtime physics consumption, and coordinate-system remediation remain manual.
...
- Unity text-YAML `.unity` conversion now maps `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs, preserves source file IDs, converts local quaternions to deterministic Euler degrees, maps enabled valid perspective Camera optics and BoxCollider geometry while rejecting disabled instances, resolves MonoBehaviour GUID bindings through `.cs.meta`, and reports each mapped coverage class separately
