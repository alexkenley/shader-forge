...
- `AnimationSystem` does not own prefab lookup; evaluator-side prefab/render-procgeo resolution uses `DataFoundation`. Optional schema-v2 joint limits and diagnostic capsules parse strictly and cook deterministically; independent optional prefab box collision parses strictly into `DataFoundation` snapshot truth. Numeric evaluation remains open
- `AnimationSystem` samples schema-v2 clip poses deterministically and the sampled evaluator applies v2 two-bone IK; both evaluators resolve exact authored visual-box corners through `DataFoundation`, while Assets displays those outlines and exact authored samples. No current path renders attachments or treats the visual box as collision geometry, and there are no available joint-limit/clipping diagnostics, camera/capture, or review evidence
...
- generic bake/runtime consumption, sampled MCP evidence, numeric joint/capsule/collision diagnostics, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; authored joint/capsule schema and cooking, strict prefab collision source truth, prefab-bound authored visual-box evidence, native clip sampling, v2 two-bone IK, transient sessiond evidence, attachment preview/review/apply/undo, and exact non-review Assets rest/sampled schematics are implemented
...
- Phase 5.73 continuation: transform the implemented authored skeleton metadata and explicit prefab collision box into numeric joint-limit/capsule clipping diagnostics; then add validation/recapture/review operations, native cameras/capture, and immutable review packets before widening typed `sf-mcp` spatial reads/review, without introducing a second attachment path
