...
Status:
- native schema/query/cook/rest-schematic evaluation, compatible v1/strict v2 profiles, deterministic schema-v2 clip sampling and sampled two-bone IK, strict CLI queries, semantic attachment operations, and constrained Assets tuner with exact rest/sampled rig schematics and resolved pole projection implemented; rendered capture/review workflow deferred
...
- `evaluate-rest` is an unsolved schematic query only: `pose.sampled=false`; it resolves exact prefab-bound authored visual-box corners through `DataFoundation`, while solved IK, collision geometry, joint-limit, and clipping diagnostics remain unavailable; the output is not review evidence
...
- the Assets third pane consumes only bounded native evaluator reports, binds authored/candidate evidence to exact revision/operation identity, and offers Front X/Y, Side Z/Y, and Top X/Z projections plus exact coordinates and unavailable diagnostics; cached visuals never follow un-evaluated draft edits
...
- `AnimationSystem` does not own prefab lookup; evaluator-side prefab/render-procgeo resolution uses `DataFoundation`. Optional schema-v2 joint limits and diagnostic capsules parse strictly and cook deterministically; independent optional prefab box collision parses strictly into `DataFoundation` snapshot truth. Rest/sample evaluation now reports non-mutating rest-relative cone-swing and signed-twist violations per authored limited bone; serialization and capsule/item clipping remain open
- `AnimationSystem` samples schema-v2 clip poses deterministically and the sampled evaluator applies v2 two-bone IK; both evaluators resolve exact authored visual-box corners through `DataFoundation`, while Assets displays those outlines and exact authored samples. Native joint-limit diagnostics exist but are not yet serialized through the product surfaces. No current path renders attachments or treats the visual box as collision geometry, and there are no available clipping diagnostics, camera/capture, or review evidence
...
- generic bake/runtime consumption, sampled MCP evidence, serialized joint-limit presentation, numeric capsule/collision diagnostics, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; authored joint/capsule schema and cooking, native numeric cone-twist diagnostics, strict prefab collision source truth, prefab-bound authored visual-box evidence, native clip sampling, v2 two-bone IK, transient sessiond evidence, attachment preview/review/apply/undo, and exact non-review Assets rest/sampled schematics are implemented
...
- Phase 5.73 continuation: serialize the implemented native joint-limit diagnostics through CLI/sessiond/Assets, then transform authored diagnostic capsules and the explicit prefab collision box into clipping diagnostics; add validation/recapture/review operations, native cameras/capture, and immutable review packets before widening typed `sf-mcp` spatial reads/review, without introducing a second attachment path
