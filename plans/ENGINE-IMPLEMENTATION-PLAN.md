...
- Phase 5.73 has compatible v1 skeleton/clip/attachment loading, strict v2 skeleton/socket/attachment validation with item-space pole and semantic arm/palm capability, deterministic schema-v2 clip sampling and sampled two-bone secondary-hand IK, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic v1/v2 spatial cooker, strict native/`engine` CLI rest/sample queries, a revision-safe transient rest-only sessiond GET, exact rest baseline/candidate preview evaluations, a lease-gated semantic attachment operation, a constrained Assets tuner with an exact native-evaluated rest-rig schematic and resolved v2 pole projection, and lease-gated `sf-mcp` preview/review/apply/undo. Sampled IK reports physical reachability plus separate reach/contact/angular tolerance truth; v1 remains pre-IK and every sampled result remains non-review evidence. Sessiond, Assets, and MCP remain rest-only. Generic bake/runtime consumption, sampled sessiond/shell evidence, prefab/item geometry, joint/clipping diagnostics, native camera/capture/review packets, and typed MCP validation/review resources remain ahead.
...
- `GET /api/spatial/attachment/evaluate` now stages exact current authored animation bytes, binds the evaluator report to the source's validator-selected ID and schema version, and rechecks the requested revision without a lease or journal write
...
- generic bake/runtime consumption, sampled sessiond/shell evidence, prefab/item-mesh integration, joint/capsule/clipping diagnostics, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; native clip sampling, v2 two-bone IK, attachment preview/review/apply/undo, and the unsampled Assets schematic are implemented
