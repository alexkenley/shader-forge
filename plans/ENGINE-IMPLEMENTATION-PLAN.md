...
- Phase 5.9 has now started through `ai/providers.toml`, shared provider inspection/smoke-test code under `tools/shared/engine-ai-service.mjs`, optional Ollama plus OpenRouter/Kimi K3 and GLM 5.2 execution, `engine_sessiond` AI inspection/test plus queued job routes, `engine ai providers|test|request`, a shell-side workspace AI status card, and `test:ai-scaffold`.
...
- additional hosted-provider execution, cumulative usage persistence/budgets/spend accounting, and secure key storage beyond environment-backed OpenRouter BYOK plus its current per-request output ceiling and usage evidence
