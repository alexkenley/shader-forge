...
- audio subsystem
- animation subsystem
- spatial authoring and attachment-tuning subsystem (native schema/query/cook/rest-schematic, compatible v1/strict v2 attachment profiles, deterministic clip sampling and sampled v2 two-bone IK, strict CLI queries, and exact Assets rest/sampled rig workbench with resolved pole projection implemented; rendered review workflow deferred)
- physics subsystem
...
- [Engine Spatial Authoring Spec](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md): Phase 5.73 native schema/query/cook/rest-schematic foundation, exact Assets rest/sampled rig workbench, and deferred rendered review workflow
...
- Phase 5.73 has compatible v1 skeleton/clip/attachment loading, strict v2 skeleton/socket/attachment validation, deterministic clip sampling and v2 two-bone IK, native rest-relative cone-twist diagnostics serialized through CLI/sessiond/Assets, generation-safe query handles, an explicit atomic cooker, strict native/CLI rest/sample queries, revision-safe sessiond evidence, semantic attachment operations, a constrained Assets tuner, and lease-gated `sf-mcp` mutation tools. Rest/sample resolve `item_prefab` through `DataFoundation`, emit exact authored visual-box corners for one valid unambiguous box procgeo, bind animation/content/foundation revisions, and show the box in Assets without claiming collision or review truth. Authored v2 cone-twist metadata is parsed/cooked; native rest/sample evaluates rest-relative swing and signed twist without mutating or clamping pose; `shader_forge_spatial` emits one exact typed aggregate/per-bone JSON object; sessiond validates it fail-closed against evaluator bones/order/roles and recomputed truth; Assets shows diagnose-only aggregate and per-bone PASS/FAIL. Unavailable is exact `no_joint_limits_authored`. Clamp policy is rejected. Capsule/item clipping, collision evaluation, generic bake/runtime consumption, sampled MCP evidence, native camera/capture/review packets, and typed MCP validation/review resources remain ahead.
...
- Phase 5.73 now has native schema/query/cook/rest/sample foundations, strict authored cone-twist/capsule skeleton metadata, strict typed prefab box-collision source truth, deterministic v2 two-bone IK, native numeric cone-twist diagnostics serialized through `shader_forge_spatial`, fail-closed sessiond validation, diagnose-only Assets aggregate/per-bone PASS/FAIL, revision-safe transient sessiond evidence, exact baseline/candidate preview, CLI and `sf-mcp` mutation adapters, constrained shell tuning, and responsive Assets schematics. The evaluator resolves prefab render procgeo through `DataFoundation`; exact eight-corner authored visual-box evidence is staged and revision-bound with every content TOML plus the foundation manifest, validated fail-closed by sessiond and shell, and drawn without a collision or review claim. Joint/capsule values are typed and cooked; prefab collision is typed independently from render geometry. Joint limits are evaluated natively without pose mutation or clamp and serialized through the CLI/sessiond/Assets contract. Capsule/item clipping remains unevaluated. There is still no rendered item mesh, available clipping result, camera/capture evidence, or review packet. Generic bake/runtime consumption, rendered capture, review packets, operation-scoped validation/recapture, sampled MCP evidence, and typed MCP spatial resources remain deferred.
...
Status:
- native schema/query/cook/rest-schematic evaluation, compatible v1/strict v2 profiles, deterministic schema-v2 clip sampling and sampled two-bone IK, authored v2 cone-twist parse/cook plus end-to-end diagnose-only joint-limit diagnostics, strict CLI queries, semantic attachment operations, and constrained Assets tuner with exact rest/sampled rig schematics and resolved pole projection implemented; capsule/item clipping, collision evaluation, sampled MCP evidence, generic bake/runtime consumption, typed MCP review resources, and rendered capture/review workflow deferred
...
- `evaluate-rest` is an unsolved schematic query only: `pose.sampled=false`; it resolves exact prefab-bound authored visual-box corners through `DataFoundation` and emits one exact typed `diagnostics.jointLimits` object for rest-relative swing and signed twist without mutating or clamping the pose; solved IK, collision geometry, and clipping diagnostics remain unavailable; the output is not review evidence
...
- `AnimationSystem` does not own prefab lookup; evaluator-side prefab/render-procgeo resolution uses `DataFoundation`. Optional schema-v2 joint limits and diagnostic capsules parse strictly and cook deterministically; independent optional prefab box collision parses strictly into `DataFoundation` snapshot truth. Rest/sample evaluation reports non-mutating rest-relative cone-swing and signed-twist violations per authored limited bone; `shader_forge_spatial` serializes one exact typed aggregate/per-bone object; sessiond validates it fail-closed against evaluator bones/order/roles and recomputed truth; Assets shows diagnose-only PASS/FAIL. Unavailable is exact `no_joint_limits_authored`. Clamp policy is rejected. Capsule/item clipping remains open
- `AnimationSystem` samples schema-v2 clip poses deterministically and the sampled evaluator applies v2 two-bone IK; both evaluators resolve exact authored visual-box corners through `DataFoundation` and emit the same exact typed joint-limit object, while Assets displays those outlines, exact authored samples, and diagnose-only joint-limit PASS/FAIL. No current path renders attachments or treats the visual box as collision geometry, and there are no available clipping diagnostics, camera/capture, or review evidence
...
- generic bake/runtime consumption, sampled MCP evidence, numeric capsule/collision diagnostics, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; authored joint/capsule schema and cooking, native numeric cone-twist diagnostics serialized through CLI/sessiond/Assets, strict prefab collision source truth, prefab-bound authored visual-box evidence, native clip sampling, v2 two-bone IK, transient sessiond evidence, attachment preview/review/apply/undo, and exact non-review Assets rest/sampled schematics are implemented
...
9. audio system
10. animation system
11. spatial authoring and attachment tuning (native schema/query/cook/rest-schematic, deterministic clip sampling and v2 two-bone IK, strict CLI queries, and exact rest/sampled Assets workbench with resolved pole projection implemented; capture/review and typed MCP widening continue before broad World/Assets visual polish)
...
- Phase 5.73 continuation: transform authored diagnostic capsules and the explicit prefab collision box into clipping diagnostics; add validation/recapture/review operations, native cameras/capture, and immutable review packets before widening typed `sf-mcp` spatial reads/review, without introducing a second attachment path
