...
- Phase 5.8 groundwork now exists through fixture-backed Shader Forge project skeleton conversion, exact startup-setting translation/provenance, deterministic duplicate-scene disambiguation, Unity and Godot text-scene hierarchy/local-transform mapping, fail-closed unresolved startup handling, deterministic no-declaration fallback, and production-bake validation; full asset/material import, component/prefab-instance mapping, conflict-safe reimport, exporter-assisted Unreal actor extraction, and shell-side migration inspection still remain
...
- Unity text-YAML `.unity` conversion now maps `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs, preserves source file IDs, converts local quaternions to deterministic Euler degrees, maps valid perspective Camera optics, and reports mapped scene entities
...
- Unity prefab-instance/non-camera-component/orthographic-camera/coordinate-system conversion, Unreal scene hierarchy and transform extraction, plus Godot transform matrices, resource instances, and component payloads
