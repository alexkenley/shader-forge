...
- audio subsystem
- animation subsystem
- spatial authoring and attachment-tuning subsystem (native schema/query/cook/rest-schematic plus exact Assets rest-rig workbench implemented; sampled/rendered review workflow deferred)
- physics subsystem
...
- Phase 5.73 has started with compatible v1 skeleton loading, strict v2 skeleton/socket and v1 attachment-profile validation, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic spatial cooker, a deterministic rest-pose schematic evaluator, a revision-safe transient sessiond GET, exact baseline/candidate preview evaluations, a lease-gated semantic attachment operation, a strict sessiond-backed CLI adapter, a constrained Assets tuner with an exact native-evaluated rest-rig schematic, and lease-gated `sf-mcp` preview/review/apply/undo. Assets binds evidence to revision and operation identity, never moves cached geometry optimistically, and reconciles active operation events through App's one SSE subscription. Evaluations are unsampled, unpersisted, and not review evidence; generic bake/runtime consumption, sampling, IK, prefab/item geometry, joint/clipping diagnostics, native camera/capture/review packets, and typed MCP validation/review resources remain ahead.
...
Status:
- native schema/query/cook/rest-schematic evaluation, semantic attachment operations, CLI adapter, and constrained Assets tuner with exact rest-rig schematic implemented; sampled/rendered capture/review workflow deferred
...
- `AnimationSystem` does not sample clip poses, evaluate IK, render attachments, or produce mesh/joint-limit/clipping diagnostics; its rest evaluator and the Assets schematic report those capabilities unavailable and do not claim camera/capture/review evidence
...
- generic bake/runtime consumption, sampling/IK, prefab/item-mesh integration, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; attachment preview/review/apply/undo and the unsampled Assets schematic are implemented
...
9. audio system
10. animation system
11. spatial authoring and attachment tuning (native schema/query/cook/rest-schematic and exact unsampled Assets workbench implemented; sampling, IK, capture/review, and typed MCP widening continue before broad World/Assets visual polish)
