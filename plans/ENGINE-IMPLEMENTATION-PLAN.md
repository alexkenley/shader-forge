...
- Phase 5.9 has now started through `ai/providers.toml`, shared provider inspection/smoke-test code under `tools/shared/engine-ai-service.mjs`, optional Ollama plus OpenRouter/Kimi K3 and GLM 5.2 execution, `engine_sessiond` AI inspection/test, queued job and usage-summary routes, `engine ai providers|test|request|usage`, a shell-side workspace AI status card, and `test:ai-scaffold`.
...
Still ahead inside this phase:
- durable queued request history, retry, and fallback behavior beyond the current process-scoped submit/status/cancel/event lifecycle
