...
- audio subsystem
- animation subsystem
- spatial authoring and attachment-tuning subsystem (native schema/query/cook/rest-schematic slice implemented; visual evidence workflow deferred)
- physics subsystem
...
- [Engine Spatial Authoring Spec](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md): Phase 5.73 native schema/query/cook/rest-schematic foundation and deferred visual tuning workflow
...
- Phase 5.73 has started with compatible v1 skeleton loading, strict v2 skeleton/socket and v1 attachment-profile validation, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic spatial cooker, a deterministic rest-pose schematic evaluator, a revision-safe transient sessiond GET, exact baseline/candidate preview evaluations, a lease-gated semantic attachment operation, a strict sessiond-backed CLI adapter, a constrained Assets tuner, and lease-gated `sf-mcp` preview/review/apply/undo. Evaluations are unsampled, unpersisted, and not review evidence; generic bake/runtime consumption, sampling, IK, item geometry, joint/clipping diagnostics, rendering/capture/review packets, and typed MCP validation/review resources remain ahead.
...
- Phase 5.73 now has a native schema/query/cook/rest-schematic foundation with isolated v2 skeleton/socket and attachment fixtures, one explicit deterministic derived payload, a read-only rest-pose attachment evaluator, a revision-safe transient sessiond GET, exact transient baseline/candidate evaluations in attachment preview, CLI and `sf-mcp` preview/approve/reject/apply/undo adapters, and constrained shell tuning. GET requires exact bytes/revision and creates no journal state; preview binds validator-selected IDs and rechecks its lease after evaluation before creating the generic operation. The normal authored/runtime lane remains v1-compatible. Rest evaluation has `pose.sampled=false`, no item mesh/IK/joint/clipping evidence, and is not a review packet; prefab existence, joint/capsule parsing, generic bake/runtime consumption, sampling, IK, rendering/capture, review packets, operation-scoped validation/recapture, and typed MCP spatial resources remain deferred.
...
Status:
- native schema/query/cook/rest-schematic evaluation, semantic attachment operations, CLI adapter, and constrained Assets tuner implemented; visual capture/review workflow deferred
...
- `AnimationSystem` does not sample clip poses, evaluate IK, render attachments, or produce mesh/joint-limit/clipping diagnostics; its rest evaluator reports those capabilities unavailable
...
- generic bake/runtime consumption, sampling/IK, operation-scoped validate/recapture/review packets, screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; attachment preview/review/apply/undo is implemented
...
9. audio system
10. animation system
11. spatial authoring and attachment tuning (native schema/query/cook/rest-schematic slice started; visual evidence workflow after the operation layer and before broad World/Assets visual polish)
...
- Phase 5.73 continuation: build from the deterministic rest-pose schematic into real sampling, IK, available geometry/joint/clipping diagnostics, validation/recapture/review operations, and the shared tuning/capture workflow; integrate the explicit cooker with generic bake/runtime without letting World/Assets polish introduce a second attachment path
