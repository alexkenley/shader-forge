...
- the shell `Workspace` panel now exposes the active policy source, supported authored hot-reload roots, recent tracked artifacts, and pending code-trust approvals with inline approve/deny actions
- deterministic coverage now exists through `scripts/test-engine-code-trust-scaffold.mjs` for approval queue listing, approval/deny flows, approved deferred execution, allowed authored-content hot reload, and rejected assistant-triggered engine load transitions
