...
- audio subsystem
- animation subsystem
- spatial authoring and attachment-tuning subsystem (native schema/query/cook/rest-schematic, compatible v1/strict v2 attachment profiles, deterministic clip sampling and sampled v2 two-bone IK, strict CLI queries, and exact Assets rest-rig workbench with resolved pole projection implemented; sampled shell evidence/rendered review workflow deferred)
- physics subsystem
...
- [Engine Spatial Authoring Spec](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md): Phase 5.73 native schema/query/cook/rest-schematic foundation, exact Assets rest-rig workbench, and deferred sampled/rendered review workflow
...
- Phase 5.73 has compatible v1 skeleton/clip/attachment loading, strict v2 skeleton/socket/attachment validation with item-space pole and semantic arm/palm capability, deterministic schema-v2 clip sampling and sampled two-bone secondary-hand IK, generation-safe query handles, isolated humanoid/weapon fixtures, deterministic native validation, an explicit atomic v1/v2 spatial cooker, strict native/`engine` CLI rest/sample queries, revision-safe transient sessiond rest/sample GETs with full animation-input manifests, exact rest baseline/candidate preview evaluations, a lease-gated semantic attachment operation, a constrained Assets tuner with an exact native-evaluated rest-rig schematic and resolved v2 pole projection, and lease-gated `sf-mcp` preview/review/apply/undo. Sampled IK reports physical reachability plus separate reach/contact/angular tolerance truth; v1 remains pre-IK and every sampled result remains non-review evidence. Assets and MCP remain rest-only. Generic bake/runtime consumption, sampled shell/MCP evidence, prefab/item geometry, joint/clipping diagnostics, native camera/capture/review packets, and typed MCP validation/review resources remain ahead.
...
- Phase 5.73 now has a native schema/query/cook/rest-schematic foundation with isolated v2 skeleton/socket/attachment and sampled clip fixtures, deterministic v1/v2 derived payloads, deterministic sampled v2 two-bone IK, strict native/`engine` CLI rest/sample queries, revision-safe transient sessiond rest/sample GETs with complete source-revision binding, exact transient rest baseline/candidate evaluations in attachment preview, CLI and `sf-mcp` mutation adapters, constrained shell tuning, and an accessible responsive Assets rest schematic with resolved v2 pole projection. Sampled evaluation requires an exact authored envelope time; v2 two-hand output applies primary attachment and secondary IK with numeric reach/contact/angular truth, while v1 stays pre-IK and one-hand marks IK not applicable. The shell/MCP evidence lane remains rest-only, with no item mesh, sampled solved pose, joint/clipping result, camera/capture evidence, or review packet; prefab existence, joint/capsule parsing, generic bake/runtime consumption, sampled operation evidence, rendered capture, review packets, operation-scoped validation/recapture, and typed MCP spatial resources remain deferred.
...
Status:
- native schema/query/cook/rest-schematic evaluation, compatible v1/strict v2 profiles, deterministic schema-v2 clip sampling and sampled two-bone IK, strict CLI queries, semantic attachment operations, and constrained Assets tuner with exact rest-rig schematic/resolved pole projection implemented; sampled shell evidence/rendered capture/review workflow deferred
...
- `AnimationSystem` samples schema-v2 clip poses deterministically and the sampled spatial attachment evaluator applies v2 two-bone secondary-hand IK; the rest evaluator and Assets schematic remain unsolved, and no current path renders attachments or produces mesh/joint-limit/clipping diagnostics, camera/capture, or review evidence
...
- generic bake/runtime consumption, sampled shell/MCP evidence, prefab/item-mesh integration, joint/capsule/clipping diagnostics, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; native clip sampling, v2 two-bone IK, transient sampled sessiond evidence, attachment preview/review/apply/undo, and the unsampled Assets schematic are implemented
...
9. audio system
10. animation system
11. spatial authoring and attachment tuning (native schema/query/cook/rest-schematic, deterministic clip sampling and v2 two-bone IK, strict CLI queries, and exact unsampled Assets workbench with resolved pole projection implemented; sampled shell evidence, capture/review, and typed MCP widening continue before broad World/Assets visual polish)
...
- Phase 5.73 continuation: feed the implemented full-input-bound sampled sessiond evidence into the shell, then add prefab/item geometry, available joint/clipping diagnostics, validation/recapture/review operations, native cameras/capture, and immutable review packets; integrate the explicit cooker with generic bake/runtime and widen typed `sf-mcp` spatial reads/review only after those engine operations exist, without introducing a second attachment path
