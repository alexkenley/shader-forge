...
- [Engine Spatial Authoring Spec](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md): Phase 5.73 native schema/query/cook/rest-schematic foundation, operation-scoped validation, exact Assets rest/sampled rig workbench, and deferred rendered review workflow
...
Status:
- native schema/query/cook/rest-schematic evaluation, compatible v1/strict v2 profiles, deterministic schema-v2 clip sampling and sampled two-bone IK, authored v2 cone-twist/capsule parse/cook, end-to-end diagnose-only joint-limit and capsule/item surface-clearance diagnostics, deterministic depth-tested native close/player-camera sample capture, strict CLI queries, typed revision-bound MCP rest/sample reads, lease-free operation validation through thin CLI/MCP adapters, semantic attachment mutation revalidation, constrained Assets tuning, lease-gated sessiond recapture, and atomic immutable review publication/reads implemented; generic bake/runtime consumption, runtime collision/physics integration, and typed CLI/MCP/shell review adapters deferred
...
- generic bake/runtime consumption, runtime collision/physics integration, rendered spatial tuning, and typed CLI/MCP/shell review adapters are still deferred; authored joint/capsule schema and cooking, native numeric joint-limit and capsule/item surface-clearance diagnostics through CLI/sessiond/Assets, strict prefab collision source truth, native clip sampling, v2 two-bone IK, deterministic native close/player-camera PNG capture, transient sessiond evidence, lease-free operation validation through CLI/`sf-mcp`, native mutation revalidation, read-only revision-bound `spatial_attachment_read`, attachment preview/review/apply/undo, exact sessiond recapture/immutable packet reads, and non-review Assets rest/sampled schematics are implemented
...
- Phase 5.73 continuation: add thin CLI/MCP/shell adapters and typed review resources over the implemented sessiond recapture, revision-bound frame capture, and immutable packet routes without introducing a second attachment path
