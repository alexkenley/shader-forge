...
- `AnimationSystem` does not own prefab lookup; evaluator-side prefab/render-procgeo resolution uses `DataFoundation`. Optional schema-v2 joint limits and diagnostic capsules now parse strictly and cook deterministically; explicit prefab collision parsing and numeric evaluation remain open
...
- generic bake/runtime consumption, sampled MCP evidence, explicit item collision geometry, numeric joint/capsule/clipping diagnostics, operation-scoped validate/recapture/review packets, native camera/screenshot capture, rendered spatial tuning, and the corresponding typed `sf-mcp` validation/review tools are still deferred; authored joint/capsule schema and cooking, prefab-bound authored visual-box evidence, native clip sampling, v2 two-bone IK, transient sessiond evidence, attachment preview/review/apply/undo, and exact non-review Assets rest/sampled schematics are implemented
...
- Phase 5.73 continuation: add explicit item collision geometry and numeric joint-limit/capsule clipping diagnostics on top of the implemented authored skeleton diagnostic metadata and prefab-bound visual geometry; then add validation/recapture/review operations, native cameras/capture, and immutable review packets before widening typed `sf-mcp` spatial reads/review, without introducing a second attachment path
