...
- Phase 5.9 has now started through `ai/providers.toml`, shared provider inspection/smoke-test code under `tools/shared/engine-ai-service.mjs`, optional Ollama plus OpenRouter/Kimi K3 and GLM 5.2 execution, `engine_sessiond` AI inspection/test plus queued job/history/usage routes, `engine ai providers|test|request|submit|jobs|status|cancel|history|usage`, a shell-side workspace AI status card, and `test:ai-scaffold`.
