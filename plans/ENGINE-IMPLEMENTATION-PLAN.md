...
- Phase 5.8 now maps active Unity text-YAML `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs with source file IDs and local transforms. `m_IsActive: 0` objects and their hierarchy subtrees are omitted. Enabled valid perspective `Camera` optics and non-trigger `BoxCollider` center/size geometry reuse strict prefab components while disabled components and trigger colliders fail closed, and `MonoBehaviour` GUIDs resolve through `.cs.meta` into binding-specific script-porting provenance; prefab instances, other components, disabled camera/collider and trigger-collider preservation, orthographic cameras, layer/material collider semantics, assets, script behavior/fields, runtime physics consumption, and coordinate-system remediation remain manual.
...
- Unity text-YAML `.unity` conversion now maps active `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs, preserves source file IDs, converts local quaternions to deterministic Euler degrees, omits `m_IsActive: 0` hierarchy subtrees, maps enabled valid perspective Camera optics and non-trigger BoxCollider geometry while rejecting disabled components and trigger colliders, resolves MonoBehaviour GUID bindings through `.cs.meta`, and reports each mapped coverage class separately
