...
Implemented specialized slice: attachment profiles open in a three-pane workbench with profile selection, constrained primary-grip tuning, and native-evaluated rest/sampled rig schematics. Browsing and authored sample evaluation acquire no lease; editing begins only after explicit exact-profile coordination. Authored rest evidence is path/revision/manifest-bound, authored samples additionally bind exact phase/clip/time/full input manifest, and candidate rest evidence is operation/base/proposed/state-bound. Front X/Y, Side Z/Y, and Top X/Z show only evaluator frames, with exact coordinates, diagnostics, accessible status/error text, and responsive stacking. Draft edits never move cached geometry. Sampled v1 two-hand evidence remains pre-IK and v2 shows applied-IK reach/contact/angular truth. Every schematic is not review evidence and has no item mesh, available joint-limit/clipping result, camera, or capture.
...
| Spatial authoring | foundation implemented: skeleton sockets, compatible v1/strict v2 attachment profiles, deterministic schema-v2 clip sampling and v2 sampled two-bone secondary-hand IK, strict native/CLI and revision-safe transient sessiond rest/sample queries with complete source-revision binding, exact rest baseline/candidate preview reports, semantic operations, constrained Assets tuning with exact three-projection rest/sampled rig schematics and resolved v2 pole projection, and shared CLI/MCP apply. Schema-v1 two-hand samples remain explicitly pre-IK. Sampled MCP evidence, prefab/item geometry, available joint/clipping diagnostics, native camera/capture, immutable review packets, and typed MCP spatial read/review remain open; proxy-card rendering and every current Assets schematic are not review evidence. |
...
Status: native schema/query/cook/rest-schematic evaluation, deterministic schema-v2 clip sampling and sampled two-bone secondary-hand IK, strict native/CLI plus revision-safe transient sessiond rest/sample queries, exact rest baseline/candidate preview evaluation, semantic attachment operations, constrained Assets tuner with exact rest/sampled rig schematics and resolved v2 pole projection, and first `sf-mcp` mutation adapter implemented. Native/sessiond sampled evaluation requires an exact authored phase/time, binds every staged animation input revision, applies primary attachment, and applies palm-effector IK for v2 two-hand profiles with separate physical reach, contact, and angular truth; v1 two-hand output remains explicitly pre-IK. Assets now consumes both rest and exact authored sampled evidence with native phase/time selects, full-manifest binding, stale-response guards, and explicit non-review labels; candidate and MCP evidence remain rest-only. Prefab geometry, available joint/clipping diagnostics, rendered capture, immutable review packets, sampled MCP evidence, and typed MCP spatial review/read surfaces remain open. Canonical contract: [ENGINE-SPATIAL-AUTHORING-SPEC.md](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
...
18. next: add prefab/item geometry, available joint/clipping diagnostics, native cameras/capture, and immutable review packets before exposing typed MCP spatial read/validation/recapture/review adapters; widen other MCP mutation families only when they carry equivalent resource keys
