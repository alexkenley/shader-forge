...
| Spatial authoring | foundation implemented: skeleton sockets, compatible v1/strict v2 profiles, deterministic sampling and v2 two-bone IK, strict native/CLI and revision-safe sessiond rest/sample queries, exact baseline/candidate preview, semantic operations, constrained Assets tuning, shared CLI/MCP apply, and exact prefab-bound authored visual-box evidence through `DataFoundation`. Animation/content/foundation revisions are staged together and Assets draws the native box outline without claiming collision or review truth. Explicit collision geometry, skeleton capsules, available joint/clipping diagnostics, sampled MCP evidence, native camera/capture, immutable review packets, and typed MCP spatial read/review remain open. |
...
19. next: add explicit item collision geometry, skeleton capsules, joint limits, available clipping diagnostics, native cameras/capture, and immutable review packets before exposing typed MCP spatial read/validation/recapture/review adapters
