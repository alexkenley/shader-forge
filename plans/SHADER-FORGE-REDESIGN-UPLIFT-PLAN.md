...
| Spatial authoring | foundation implemented: skeleton sockets, compatible v1/strict v2 profiles, strict optional cone-twist/capsule metadata with deterministic cooking, strict optional prefab-only box collision in `DataFoundation`, deterministic sampling and v2 two-bone IK, strict native/CLI and revision-safe sessiond rest/sample queries, exact baseline/candidate preview, lease-free operation validation through CLI/MCP, native apply/undo revalidation, deterministic native close-camera sample rendering, constrained Assets tuning, shared CLI/MCP apply, exact independent visual/collision geometry, diagnose-only joint-limit and capsule/item surface-clearance evidence through Assets, and typed revision-bound MCP rest/sample reads. Animation/content/foundation revisions are staged together. `capture-sample` writes four depth-tested clean PNGs from posed bone geometry and the authored procgeo box, but does not publish review evidence. A strict authored runtime perspective camera exists; capture must still revision-bind it and may not use the runtime fallback. Runtime collision/physics integration, player-camera capture, immutable review packets, and recapture/review adapters remain open. |
...
Status: native schema/query/cook/rest/sample evaluation, strict authored cone-twist/capsule metadata, strict optional prefab box-collision source truth, deterministic v2 two-bone IK, native numeric joint-limit and capsule/item surface-clearance diagnostics, deterministic native close-camera sample rendering, fail-closed sessiond/Assets validation, revision-safe evidence, exact baseline/candidate preview, lease-free operation validation through CLI and `sf-mcp`, native apply/undo revalidation through one semantic dispatcher, constrained Assets tuning, the first `sf-mcp` mutation adapter, and read-only revision-bound `spatial_attachment_read` are implemented. Sessiond stages and revision-binds animation, all content TOML, and the foundation manifest; operation validation verifies private proposed bytes, evaluates rest plus bounded requested samples, rejects manifest/CAS drift, and journals only controlled counts/failures. V1 two-hand samples remain pre-IK; v2 exposes physical reach plus contact/angular/joint/clipping truth. `capture-sample` produces four clean depth-tested PNG views and bounded camera/bounds/lighting metadata, but no review packet. A strict root `player_camera` prefab component supplies runtime perspective truth; review capture must still revision-bind it and cannot use the ordinary runtime fallback. Runtime collision/physics integration, player-camera capture, immutable review packets, generic bake/runtime consumption, recapture, and typed review adapters remain open. Canonical contract: [ENGINE-SPATIAL-AUTHORING-SPEC.md](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
...
25. in progress: deterministic native close-camera sample capture is implemented; add authored-camera capture, recapture lease/revision orchestration, immutable review packets, and typed review adapters
