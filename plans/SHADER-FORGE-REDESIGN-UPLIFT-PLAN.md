...
- implemented read-only, idempotent `spatial_attachment_read` for exact revision-bound rest or authored sampled evidence, lease-free `spatial_attachment_validate`, plus mutation tools `spatial_attachment_preview`, `operation_approve`, `operation_reject`, and spatial-only `operation_apply` / `operation_undo`; typed review/recapture tools remain gated on their matching engine operations
