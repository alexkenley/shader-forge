...
Implemented specialized slice: attachment profiles open in a three-pane workbench with constrained tuning and native-evaluated rest/sampled rig schematics. Rest/sample evidence binds exact animation, content, and foundation revisions. The evaluator resolves `item_prefab` through `DataFoundation` and the existing prefab render-procgeo chain; Assets draws its exact visual-box outline and eight coordinates when available, and shows diagnose-only joint-limit aggregate and per-bone PASS/FAIL. Unavailable joint limits are exact `no_joint_limits_authored`. This is authored visual evidence only, never a rendered mesh, collision shape, clipping result, or review packet. Draft edits never move cached geometry; v1 two-hand samples remain pre-IK and v2 exposes applied-IK reach/contact/angular truth.
...
| Spatial authoring | foundation implemented: skeleton sockets, compatible v1/strict v2 profiles, strict optional cone-twist/capsule metadata with deterministic cooking, strict optional prefab-only box collision in `DataFoundation`, deterministic sampling and v2 two-bone IK, strict native/CLI and revision-safe sessiond rest/sample queries, exact baseline/candidate preview, semantic operations, constrained Assets tuning, shared CLI/MCP apply, and exact prefab-bound authored visual-box evidence. Animation/content/foundation revisions are staged together and Assets draws the native visual-box outline without claiming collision or review truth. Numeric joint/capsule/collision diagnostics, sampled MCP evidence, native camera/capture, immutable review packets, and typed MCP spatial read/review remain open. |
...
Status: native schema/query/cook/rest/sample evaluation, strict authored cone-twist/capsule metadata, strict optional prefab box-collision source truth, deterministic v2 two-bone IK, native numeric cone-twist diagnostics serialized through `shader_forge_spatial`, fail-closed sessiond validation, diagnose-only Assets aggregate/per-bone PASS/FAIL, revision-safe sessiond evidence, exact baseline/candidate preview, semantic operations, constrained Assets tuning, and the first `sf-mcp` mutation adapter are implemented. Native rest/sample resolve the attachment prefab through `DataFoundation` and expose exact authored visual-box corners; sessiond stages and revision-binds animation, all content TOML, and the foundation manifest; Assets validates and draws the outline with explicit non-collision/non-review labels. V1 two-hand samples remain pre-IK; v2 exposes physical reach plus contact/angular truth. Joint/capsule values cook deterministically and prefab collision is typed independently from render geometry. Authored v2 cone-twist metadata is parsed/cooked; native rest/sample evaluates rest-relative swing and signed twist without mutating or clamping pose; `shader_forge_spatial` emits one exact typed aggregate/per-bone JSON object; sessiond validates it fail-closed against evaluator bones/order/roles and recomputed truth; Assets shows diagnose-only PASS/FAIL. Unavailable is exact `no_joint_limits_authored`. Clamp policy is rejected. Capsule/item clipping is not evaluated. Collision evaluation, rendered capture, immutable review packets, sampled MCP evidence, generic bake/runtime consumption, and typed MCP spatial review/read surfaces remain open. Canonical contract: [ENGINE-SPATIAL-AUTHORING-SPEC.md](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
...
22. next: transform authored diagnostic capsules and the explicit prefab collision box into clipping diagnostics, then add native cameras/capture and immutable review packets before exposing typed MCP spatial read/validation/recapture/review adapters
