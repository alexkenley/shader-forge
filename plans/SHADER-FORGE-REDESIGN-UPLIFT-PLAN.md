...
Status: native schema/query/cook/rest/sample evaluation, strict authored cone-twist/capsule metadata, strict optional prefab box-collision source truth, deterministic v2 two-bone IK, native numeric joint-limit and capsule/item surface-clearance diagnostics, fail-closed sessiond/Assets validation, revision-safe evidence, exact baseline/candidate preview, semantic operations, constrained Assets tuning, the first `sf-mcp` mutation adapter, and read-only revision-bound `spatial_attachment_read` are implemented. Native rest/sample resolve render procgeo and collision independently through `DataFoundation`; sessiond stages and revision-binds animation, all content TOML, and the foundation manifest; Assets validates and draws the visual box, collision OBB, and capsule segments with explicit diagnose-only/non-review labels. V1 two-hand samples remain pre-IK; v2 exposes physical reach plus contact/angular/joint/clipping truth. `shader_forge_spatial` emits exact aggregates and stable per-bone/per-capsule records without mutating or clamping pose; sessiond and Assets recompute their geometry and derived truth fail closed. Missing inputs are exact unavailable states and tangency is clear. The MCP read exposes exact rest or authored sampled evaluation plus sorted source revisions without leases or operations. Runtime collision/physics integration, rendered camera/capture, immutable review packets, generic bake/runtime consumption, operation-scoped validation/recapture, and typed MCP validation/review surfaces remain open. Canonical contract: [ENGINE-SPATIAL-AUTHORING-SPEC.md](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
...
23. next: add operation-scoped validation/recapture, native cameras/capture, immutable review packets, and typed MCP validation/review adapters
