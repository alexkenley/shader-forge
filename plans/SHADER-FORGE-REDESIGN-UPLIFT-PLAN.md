...
- `sf-mcp` now exposes lease-gated spatial attachment and whole scene/prefab preview/review/apply/undo workflows through the shared operation contract; generic mutation remains disabled
...
- prefab/scene/runtime now share a strict authored perspective-camera source: one root or parented `player_camera` drives true composed-transform/FOV/clip projection, ordinary scenes retain an explicit legacy fallback, and stale quickloads fail closed; deterministic native close/player-camera spatial capture and session-owned immutable review packets consume that same source without the legacy fallback
...
- implemented read-only, idempotent `project_diagnostics` and `spatial_attachment_read`, lease-free `spatial_attachment_validate`, strict `scene_asset_preview`, spatial/review tools, generic `operation_approve` / `operation_reject`, and semantic `operation_apply` / `operation_undo`; broader tools remain gated on matching engine operations
...
| Spatial authoring | foundation implemented: skeleton sockets, compatible v1/strict v2 profiles, strict optional cone-twist/capsule metadata with deterministic cooking, strict optional prefab-only box collision in `DataFoundation`, deterministic sampling and v2 two-bone IK, strict native/CLI and revision-safe sessiond rest/sample queries, exact baseline/candidate preview, lease-free operation validation through CLI/MCP, native apply/undo revalidation, deterministic native close/player-camera sample rendering, constrained Assets tuning, shared CLI/MCP apply, exact independent visual/collision geometry, diagnose-only joint-limit and capsule/item surface-clearance evidence through Assets, typed revision-bound MCP rest/sample reads, exact lease-gated sessiond recapture with atomic immutable packet/capture reads, and packet-referenced rendered review presentation in Assets. Animation/content/foundation revisions are staged together. `capture-sample` writes four bounded close views plus an optional strict authored `player_camera` view; sessiond alone upgrades them into revision- and lease-bound review evidence after final recheck and atomic publish. CLI/MCP and Assets reserve, recapture, and read that workflow without taking authority. Runtime collision/physics integration remains open. |
...
Current status: the hardened text-file write operation contract is implemented in `engine_sessiond`, including journaled code-trust effects, serialized CLI provenance transitions, immutable workspace identity, append-only recovery provenance, loopback-only bind, bounded public JSON input, and exact public diffs. The shell consumes durable list/detail and event views in a review-only Activity dock; World and `sf-mcp` consume revision-bound lease-gated whole scene/prefab save/create/duplicate; and `sf-mcp` also consumes the spatial attachment workflow. Multi-file change sets, Activity apply/undo coordination, and further MCP mutation families remain open.
...
Status: native schema/query/cook/rest/sample evaluation, strict authored cone-twist/capsule metadata, strict optional prefab box-collision source truth, deterministic v2 two-bone IK, native numeric joint-limit and capsule/item surface-clearance diagnostics, deterministic native close/player-camera sample rendering, fail-closed sessiond/Assets validation, revision-safe evidence, exact baseline/candidate preview, lease-free operation validation through CLI and `sf-mcp`, native apply/undo revalidation through one semantic dispatcher, constrained Assets tuning, the first `sf-mcp` mutation adapter, read-only revision-bound `spatial_attachment_read`, lease-gated sessiond recapture/immutable packet reads, thin CLI/MCP review adapters, and Assets packet publication/loading/presentation are implemented. Sessiond stages and revision-binds animation, all content TOML, and the foundation manifest; operation validation verifies private proposed bytes, while recapture expands authored phase samples, validates exact source/capture/review leases, enforces authored `player_camera` evidence without the runtime fallback for first-person/both, rechecks revisions/leases, atomically publishes, and releases leases. Assets shows only packet-referenced PNGs as immutable review evidence. V1 two-hand samples remain pre-IK; v2 exposes physical reach plus contact/angular/joint/clipping truth. Runtime collision/physics integration and generic bake/runtime consumption remain open. Canonical contract: [ENGINE-SPATIAL-AUTHORING-SPEC.md](../docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
...
Current status: the process-scoped stdio adapter now covers project diagnostics/read/coordination plus lease-gated spatial attachment and whole scene/prefab preview/review/apply/undo. The shell Activity dock consumes operation history and lease-free review events. Entity/component patching, general asset/code mutation, build/runtime tools, Activity apply/undo, and full parity scenarios remain open.
...
24. completed: expose operation validation through the thin `engine spatial validate-operation` and `spatial_attachment_validate` adapters without leases, credentials, apply, or private output
25. completed: deterministic native close/player-camera capture, lease/revision-bound atomic sessiond review packets, thin CLI/MCP recapture/read adapters, the typed MCP review resource, and Assets packet publication/loading/presentation
26. completed: expose selected-workspace live diagnostics plus strict whole scene/prefab save/create/duplicate preview and shared approve/apply/undo through `sf-mcp`, with exact target/source lease coverage and no generic file-write bypass
