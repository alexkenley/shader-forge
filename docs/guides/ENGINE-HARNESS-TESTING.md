...
| `scripts/test-engine-viewer-bridge.mjs` | Starts the local backend in-process, validates viewer-bridge SSE/runtime/build flows, checks stale lifecycle, World-selection, and session-panel response guards, and verifies the shell bridge surfaces remain present | deterministic |
