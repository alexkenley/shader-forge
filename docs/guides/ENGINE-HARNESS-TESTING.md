...
| `scripts/test-engine-spatial-shell.mjs` | Executes the primary-grip transformer, motion-envelope parser, source-manifest guard, rest/sampled validator and projection guards; verifies exact eight-corner visual-box recomposition, fail-closed diagnose-only joint-limit parsing and per-bone presentation, authored-only samples, v1/v2 IK truth, evidence binding, stale guards, accessible Assets mounting, operation-only routing, explicit locking, and credential-free evaluation GETs | deterministic |
