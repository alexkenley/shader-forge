...
| `scripts/test-engine-ai-scaffold.mjs` | Starts sessiond plus a loopback provider stub and validates provider inspection, deterministic fake and Ollama-compatible requests, authenticated OpenRouter/Kimi K3 and GLM 5.2 requests, bounded retry/fallback, queued lifecycle/cancellation, durable metadata history and token usage, read-only tool/skill registry discovery and validation, endpoint pinning, credential exclusion, fail-closed provider names/types/output limits, request token ceilings, normalized usage, and the 1 MiB response bound through backend and CLI surfaces | deterministic |
