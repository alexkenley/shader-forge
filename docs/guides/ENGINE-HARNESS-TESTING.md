...
| `scripts/test-engine-spatial-authoring-scaffold.mjs` | Builds an isolated animation root, compiles the native parser, and validates v1/v2 skeleton, socket, attachment-profile, cross-reference, transactional reload, typed-handle, schema-v1 clip compatibility, deterministic schema-v2 pose sampling, exact envelope sampling, attachment composition, and procedural-layer truth; WSL `g++` is required on Windows | deterministic native |
| `scripts/test-engine-spatial-tool.mjs` | Requires a native compiler, compiles the production `shader_forge_spatial` command, and checks deterministic validation, UTF-8 input enforcement, byte-stable cooking, rest and sampled attachment geometry with pre-IK two-hand output, strict native/`engine` CLI surfaces, unchanged rest shape, truth labels, transform composition, and failure behavior; WSL `g++` is required on Windows and `g++` elsewhere | deterministic native |
