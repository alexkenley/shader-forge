...
| `scripts/test-engine-spatial-authoring-scaffold.mjs` | Builds an isolated animation root, compiles the native parser/evaluator, and validates v1/v2 skeleton, socket, compatible v1/strict v2 attachment-profile, item-space pole, chain/palm capability, cross-reference, transactional reload, typed-handle, schema-v1 clip compatibility, deterministic schema-v2 pose sampling, exact envelope sampling, two-bone IK, and procedural-layer truth; WSL `g++` is required on Windows | deterministic native |
| `scripts/test-engine-spatial-tool.mjs` | Requires a native compiler, compiles the production `shader_forge_spatial` command, and checks deterministic validation, UTF-8 input enforcement, byte-stable v1/v2 cooking, rest and sampled attachment geometry, v1 pre-IK compatibility, reachable/unreachable v2 two-bone IK, item-space pole bending, separate reach/contact/angular tolerance truth, degenerate failure, strict native/`engine` CLI surfaces, transform composition, and failure behavior; WSL `g++` is required on Windows and `g++` elsewhere | deterministic native |
| `scripts/test-engine-spatial-operations.mjs` | Starts sessiond with injected deterministic validator/evaluator seams and verifies revision-safe no-lease rest/sample GETs, exact phase/time and validator-selected ID/schema/skeleton/item/mode/perspective binding, one-hand/v1-pre-IK/v2-applied-IK truth, full animation-input revision manifests and drift conflicts, safe exact-byte staging, journal absence, bounded evaluator failures, transient rest preview evaluations including new files, post-evaluation manifest/lease rechecks, durable operation context, exact/rename lease coverage, apply/undo revision safety, CORS, and temp cleanup | deterministic |
...
| `scripts/test-engine-spatial-shell.mjs` | Executes the primary-grip transformer, independent motion-envelope parser, source-revision manifest guard, rest/sampled evaluation validator and projection guards, plus operation-reconciliation helpers; verifies exact-byte preservation, authored-only phase/time selection, v1 pre-IK and v2 applied-IK truth, exact authored/candidate/sample evidence binding, stale-request guards, fixed cached geometry, one App SSE subscription, conflict cleanup, accessible/responsive Assets mounting, operation-only routing, explicit locking, and credential-free evaluation GETs | deterministic |
