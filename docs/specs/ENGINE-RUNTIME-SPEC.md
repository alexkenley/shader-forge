...
- the normal runtime animation load path remains metadata-only: `debug_humanoid` is a three-bone `hips, spine, head` list with no sockets, no attachment profiles, and no pose sampling. A separate `shader_forge_spatial` command now validates isolated v2 skeleton/socket and attachment-profile roots through the same `AnimationSystem` and can stage one deterministic derived spatial payload; the runtime does not consume that payload and it is not a preview or gameplay integration
...
- authored camera projection, deterministic native close/player-camera spatial sample capture, session-owned immutable review packets, and Assets packet presentation exist; gameplay attachment rendering remains deferred
