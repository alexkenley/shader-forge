...
The implemented Assets rest-rig schematic is a React shell surface, not this native tooling layer. It consumes bounded v1/v2 sessiond rest evaluations and shows exact evaluator frames, coordinates, resolved v2 pole points, and unavailable diagnostics in three orthographic projections; it is unsampled and not review evidence, and it has no item mesh, sampled solved IK pose, joint-limit/clipping result, camera, or capture.
