...
The implemented Assets rest/sampled rig schematic is a React shell surface, not this native tooling layer. It consumes bounded v1/v2 sessiond evaluations and shows exact evaluator frames, coordinates, resolved v2 pole points, procedural-layer truth, sampled v2 IK diagnostics, diagnose-only joint-limit aggregate/per-bone PASS/FAIL, and diagnose-only capsule/item surface-clearance aggregate/per-capsule PASS/FAIL. It draws the authored collision OBB and diagnostic capsule segments independently from the prefab-bound render-procgeo visual box in three orthographic projections. Missing joint limits, prefab collision, or diagnostic capsules remain exact typed unavailable evidence and never silently pass. This remains schematic diagnostic evidence rather than a rendered mesh, runtime collision result, or review packet; the authored runtime camera is not revision-bound capture evidence, and candidate evaluation remains rest-only.
