...
The implemented Assets rest/sampled rig schematic is a React shell surface, not this native tooling layer. It consumes bounded v1/v2 sessiond evaluations and shows exact evaluator frames, coordinates, resolved v2 pole points, procedural-layer truth, sampled v2 IK diagnostics, and an exact prefab-bound authored visual-box outline in three orthographic projections. The box is render-procgeo evidence, not a rendered mesh or collision truth. The schematic is not review evidence and has no available joint-limit/clipping result, camera, or capture; candidate evaluation remains rest-only.
