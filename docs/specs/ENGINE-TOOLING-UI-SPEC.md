...
The implemented Assets rest/sampled rig schematic is a React shell surface, not this native tooling layer. It consumes bounded v1/v2 sessiond evaluations and shows exact evaluator frames, coordinates, resolved v2 pole points, procedural-layer truth, and sampled v2 IK reach/contact/angular diagnostics in three orthographic projections. It is not review evidence and has no item mesh, available joint-limit/clipping result, camera, or capture; candidate evaluation remains rest-only.
