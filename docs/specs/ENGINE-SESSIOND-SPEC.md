...
- `POST /api/ai/test` now runs the shared AI smoke-test path with deterministic fake-provider coverage plus optional Ollama- and OpenRouter-backed requests; OpenRouter credentials stay server-side, real request payloads carry the resolved `max_output_tokens`, and provider responses are bounded to 1 MiB
