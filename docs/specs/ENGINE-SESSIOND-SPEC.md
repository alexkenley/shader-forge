...
- `POST /api/ai/test` now runs the shared AI smoke-test path with deterministic fake-provider coverage plus optional Ollama- and OpenRouter-backed requests; OpenRouter credentials stay server-side, real request payloads carry the resolved `max_output_tokens`, provider responses are bounded to 1 MiB, and valid provider token usage is returned in a normalized shape
...
- `GET /api/package/inspect` reports export preset source, prerequisite path readiness, cooked-asset counts, last package metadata, and safe last-archive path/existence for a workspace, including whether runtime build or asset-bake prep is still needed
- `POST /api/package/run` now emits the release layout under `build/package/<preset>/`, can auto-bake missing cooked outputs, and executes the preset's built-in `archive_zip` hook to produce a sibling ZIP while reporting unsupported hooks as `declared_only`
