...
- native `engine spatial evaluate-rest|evaluate-sample` remain local commands; sessiond invokes them through transient GET paths while preview stays rest-only. The same lease-free GETs back the session-pinned MCP `spatial_attachment_read` tool. None creates a review packet or supplies durable operation-validation evidence
