...
- schema-v2 bones may author strict `cone_twist` joint limits and local diagnostic capsules; both are exposed through typed snapshots and serialized deterministically by `shader_forge_spatial cook`, while schema v1 rejects these v2-only nested tables
...
- The rest evaluator is a schematic query, not clip sampling or rendered review evidence: `pose.sampled=false`. It may carry exact prefab-bound authored visual-box corners resolved through `DataFoundation`, but those corners are not a rendered item mesh, collision geometry, or clipping truth. Authored v2 cone-twist metadata is parsed and cooked. Native rest/sample evaluation reports rest-relative swing and signed twist without mutating or clamping the pose. `shader_forge_spatial` emits one exact typed aggregate/per-bone `diagnostics.jointLimits` JSON object; sessiond validates it fail-closed against evaluator bones, order, roles, and recomputed truth; Assets shows diagnose-only aggregate and per-bone PASS/FAIL. Unavailable is exact `no_joint_limits_authored`. Clamp policy is rejected. Capsule/item clipping remains unavailable. V1 pole world space remains unresolved; v2 item-space poles resolve to world coordinates.

This is still a widening slice, not the final animation runtime. The normal authored/runtime lane remains the compatible v1 `debug_humanoid` metadata path. V2 spatial assets currently live only in fixtures. The explicit spatial cooker emits derived metadata, including optional joint limits and capsules, but is not integrated with generic `engine bake` or runtime loading. The native sampler feeds a read-only sampled attachment evaluator with deterministic v2 two-bone IK and exact reach/contact/angular/joint/clipping diagnostics resolved through `DataFoundation`. Sessiond exposes revision-bound transient reads and uses the same strict evaluator for lease-free operation validation plus native apply/undo revalidation, persisting only bounded validation counts. Native capture and sessiond recapture now publish immutable revision-bound review packets, and Assets can create, load, and present their real PNGs while keeping its diagnostic schematics separate. Runtime graph playback and generic MCP sampling do not consume this output. Blending, gameplay attachment rendering, and native preview overlay tooling remain deferred.
...
- Phase 5.73 now has its native schema/query/cook/rest-schematic slice, deterministic clip sampling, compatible v1 pre-IK output, v2 sampled two-bone IK, end-to-end diagnose-only joint-limit diagnostics, transient full-input-bound sessiond sample evidence, exact authored sample display in Assets, and attachment mutation operations; the review workflow still depends on sampled MCP consumption, item rendering, capsule/item clipping diagnostics, and capture work
