...
- The rest evaluator is a schematic query, not clip sampling or rendered review evidence: `pose.sampled=false`. It may carry exact prefab-bound authored visual-box corners resolved through `DataFoundation`, but those corners are not a rendered item mesh, collision geometry, or clipping truth. Solved IK, joint-limit, and clipping diagnostics remain unavailable. V1 pole world space remains unresolved; v2 item-space poles resolve to world coordinates.

This is still a widening slice, not the final animation runtime. The normal authored/runtime lane remains the compatible v1 `debug_humanoid` metadata path. V2 spatial assets currently live only in fixtures. The explicit spatial cooker emits derived metadata but is not integrated with generic `engine bake` or runtime loading. The native sampler feeds a read-only sampled attachment evaluator with deterministic v2 two-bone IK and exact reach/contact/angular diagnostics; the evaluator separately resolves prefab visual-box evidence through `DataFoundation`. Sessiond exposes it transiently with animation/content/foundation revision binding and Assets displays exact authored samples and visual-box outlines. Runtime graph playback and MCP do not yet consume sampled output. Collision geometry, joint-limit and diagnostic-capsule parsing, blending, attachment rendering, review capture, operation-scoped validation/recapture, and native preview tooling remain deferred.
