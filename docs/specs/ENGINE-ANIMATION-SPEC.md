...
This is still a widening slice, not the final animation runtime. The normal authored/runtime lane remains the compatible v1 `debug_humanoid` metadata path. V2 spatial assets currently live only in fixtures. The explicit spatial cooker emits derived metadata, including optional joint limits and capsules, but is not integrated with generic `engine bake` or runtime loading. The native sampler feeds a read-only sampled attachment evaluator with deterministic v2 two-bone IK and exact reach/contact/angular/joint/clipping diagnostics resolved through `DataFoundation`. Sessiond exposes revision-bound transient reads and now uses the same strict evaluator for lease-free operation validation plus native apply/undo revalidation, persisting only bounded validation counts. Assets displays exact authored samples and diagnose-only joint/clipping truth. Runtime graph playback and generic MCP sampling do not consume this output. Blending, attachment rendering, review capture/recapture, and native preview tooling remain deferred.
