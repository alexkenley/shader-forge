...
- `engine spatial evaluate-sample --attachment <id> --phase <phase> --normalized-time <value> [--animation-root animation] [--build-dir build/runtime] [--config Debug]` resolves an exact authored motion-envelope phase/time and emits the same geometry over the deterministic schema-v2 clip sampler. Its pose is `kind="clip_sample"` and `sampled=true`; primary attachment is applied and requested/applied/unavailable procedural layers are explicit. Two-hand results report secondary-hand IK unavailable and include `pre_ik_only`; one-hand results report secondary-hand IK not applicable and include `sampled_attachment_schematic_only`. Both include `not_review_evidence`. The command is read-only and creates no review artifact.
