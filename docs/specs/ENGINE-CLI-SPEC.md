...
- `engine ai test [--root <path>] [--provider <id>] [--prompt <text>] [--system <text>]` now runs a workspace-backed smoke test through the shared AI layer for deterministic fake, optional Ollama, or enabled OpenRouter providers
- `engine ai request <prompt> [--root <path>] [--provider <id>] [--system <text>]` reuses the same path for freeform prompts; the disabled-by-default `openrouter_kimi` manifest entry uses `OPENROUTER_API_KEY`, `moonshotai/kimi-k3`, and a validated `max_output_tokens` ceiling
