...
- `engine ai request <prompt> [--root <path>] [--provider <id>] [--system <text>]` reuses the same path for freeform prompts; the disabled-by-default `openrouter_kimi` manifest entry uses `OPENROUTER_API_KEY` and `moonshotai/kimi-k3`
