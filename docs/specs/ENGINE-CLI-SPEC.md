...
- `engine export inspect [--root <path>] [--preset <id>] [--package-root <path>]` prints the resolved preset, prerequisites, cooked-asset counts, last-package summary, and whether its safely bounded archive output still exists; source-controlled `default` and `release` desktop presets provide Debug and Release configurations
