...
Spatial authoring now has a native schema/query/cook slice, deterministic schema-v2 clip pose sampling, a deterministic rest-pose attachment evaluator, a revision-safe transient sessiond GET, semantic sessiond operation, strict CLI adapter, a constrained Assets primary-grip tuner with a native-evaluated rest-rig schematic, and lease-gated `sf-mcp` preview/review/apply/undo tools. GET stages and evaluates exact current authored bytes under the validator-selected attachment ID, then rechecks revision; it needs no lease and creates no operation or persisted record. Preview returns exact baseline/candidate evaluations transiently and rechecks its write lease after evaluation before journaling. Assets binds authored evidence to session/path/revision and candidate evidence to operation/base/proposed/state, never moves cached geometry on draft edits, and fails the entire schematic closed on malformed or oversized reports. Its Front X/Y, Side Z/Y, and Top X/Z projections show exact evaluator frames plus coordinate/diagnostic summaries in an accessible responsive three-pane workbench. The clip sampler is not yet connected to attachment evaluation, so the evaluator and UI remain `pose.sampled=false`; there is no item mesh, solved IK, joint-limit/clipping result, camera, capture, or review evidence. Every mutating adapter uses the same authored TOML, revisions, operation journal, and hierarchical attachment keys. Generic `engine bake` integration, runtime consumption, sampled attachment evaluation, IK, prefab geometry, capture/review packets, typed MCP spatial resources, and MCP validation/recapture remain deferred; see [ENGINE-SPATIAL-AUTHORING-SPEC.md](/mnt/s/Development/AI-Game-Engine/docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
