...
Spatial authoring now has a native schema/query/cook slice plus the first semantic sessiond operation: compatible v1 skeleton loading, strict v2 skeleton/socket validation, v1 attachment-profile validation, generation-safe query handles, isolated fixtures, deterministic validation JSON with stable attachment source paths, one atomic derived socket/profile payload through `shader_forge_spatial cook`, and lease-gated no-write attachment preview over the generic operation journal. Generic `engine bake` integration, runtime consumption, sampling, IK, capture/review packets, shell UI, and MCP remain deferred; see [ENGINE-SPATIAL-AUTHORING-SPEC.md](/mnt/s/Development/AI-Game-Engine/docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
