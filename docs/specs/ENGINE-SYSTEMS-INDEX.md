...
Spatial authoring now has a first native schema/query slice: compatible v1 skeleton loading, strict v2 skeleton/socket validation, v1 attachment-profile validation, generation-safe query handles, isolated fixtures, and a read-only `shader_forge_spatial` validator with deterministic JSON plus CLI/harness coverage. Cooking, sampling, IK, capture, tuning operations, shell UI, and MCP remain deferred; see [ENGINE-SPATIAL-AUTHORING-SPEC.md](/mnt/s/Development/AI-Game-Engine/docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
