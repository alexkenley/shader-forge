...
Spatial authoring now has a native schema/query/cook slice, deterministic schema-v2 clip pose sampling, deterministic rest and sampled attachment evaluators with pre-IK two-hand output, strict native/`engine` CLI queries, a revision-safe transient rest-only sessiond GET, semantic sessiond operation, a constrained Assets primary-grip tuner with a native-evaluated rest-rig schematic, and lease-gated `sf-mcp` preview/review/apply/undo tools. Sampled evaluation resolves an exact authored envelope phase/time and applies primary attachment over sampled bones; two-hand output reports secondary-hand IK unavailable instead of claiming it ran, while one-hand output reports it not applicable. GET stages and evaluates exact current authored bytes under the validator-selected attachment ID, then rechecks revision; it needs no lease and creates no operation or persisted record. Preview returns exact rest baseline/candidate evaluations transiently and rechecks its write lease after evaluation before journaling. Assets binds authored evidence to session/path/revision and candidate evidence to operation/base/proposed/state, never moves cached geometry on draft edits, and fails the entire schematic closed on malformed or oversized reports. Its Front X/Y, Side Z/Y, and Top X/Z projections remain rest-only and not review evidence. There is no item mesh, solved IK, joint-limit/clipping result, camera, capture, or review packet. Every mutating adapter uses the same authored TOML, revisions, operation journal, and hierarchical attachment keys. Generic `engine bake` integration, runtime consumption, IK, sampled sessiond/shell/MCP evidence, prefab geometry, capture/review packets, typed MCP spatial resources, and MCP validation/recapture remain deferred; see [ENGINE-SPATIAL-AUTHORING-SPEC.md](/mnt/s/Development/AI-Game-Engine/docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
