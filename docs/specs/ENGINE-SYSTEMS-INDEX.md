...
The shell-native `Code` workspace now owns bounded file browsing, revision-bound Monaco source/diff tabs, current-file search beside `Inspect`, dirty-draft retention, typed revision conflicts, and explicit Preview/Approve/Reject/Apply/Undo over the same durable operation journal. It never writes through `/api/files/write`; reviewed candidates are bound to exact draft bytes, and async session/tab/operation guards prevent late responses or apply/undo refreshes from erasing newer work. The preserved `web/` editor is an optional compatibility bridge rather than the default workflow.
