...
Spatial authoring now has a native schema/query/cook slice, compatible v1 plus strict v2 attachment profiles, deterministic schema-v2 clip pose sampling, deterministic rest evaluation, and sampled v2 two-bone secondary-hand IK with item-space pole, palm-effector, physical reach, and separate reach/contact/angular tolerance truth. Strict native/`engine` CLI plus revision-safe transient sessiond rest/sample queries, semantic sessiond operations, a constrained Assets primary-grip tuner with native-evaluated rest/sampled rig schematics, and lease-gated `sf-mcp` preview/review/apply/undo tools are implemented. Schema-v1 two-hand sampled output remains explicitly pre-IK. Both GETs stage exact authored bytes, bind the validator-selected attachment ID/schema/skeleton/item/mode/perspective contract and requested phase/time, expose a sorted revision manifest for every staged animation source, and fail closed on selected or dependency drift; they need no lease and create no operation or persisted record. Preview returns exact unsolved rest baseline/candidate evaluations with the same profile binding, then rechecks the full source manifest and write lease before journaling. Assets binds authored rest evidence to session/path/revision/manifest, authored samples to exact phase/clip/time/full manifest, and candidate rest evidence to operation/base/proposed/state. It offers only authored envelope samples, never moves cached geometry on draft edits, fails malformed/oversized/contradictory reports closed, projects resolved v2 poles as green rings, and shows v2 applied-IK reach/contact/angular truth. Its Front X/Y, Side Z/Y, and Top X/Z projections are not review evidence. There is no item mesh, available joint-limit/clipping result, camera, capture, or review packet. Every mutating adapter uses the same authored TOML, revisions, operation journal, and hierarchical attachment keys. Generic `engine bake` integration, runtime consumption, sampled MCP evidence, prefab geometry, joint/clipping integration, capture/review packets, typed MCP spatial resources, and MCP validation/recapture remain deferred; see [ENGINE-SPATIAL-AUTHORING-SPEC.md](/mnt/s/Development/AI-Game-Engine/docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md).
