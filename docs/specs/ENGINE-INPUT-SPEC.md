...
- runtime-side consumption of named actions such as `runtime_exit`, `reload_runtime_content`, `move_x`, `move_y`, `look_x`, `look_y`, `ui_accept`, and `ui_back`, including the current `F7` authored-content reload lane and `ui_accept` interaction triggering
