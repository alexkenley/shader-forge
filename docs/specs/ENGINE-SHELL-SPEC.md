...
- Explorer, Git, package, profiling, trust-summary, and trust-approval refreshes retain independent request generations plus active-session identity. Selecting another workspace synchronously invalidates all six lanes, including an A-to-B-to-A return, so stale data and errors cannot overwrite the active workspace
