...
The current schematic is explicitly unsampled and not review evidence. It draws evaluator bone/socket/hand/palm/target frames, item origin/orientation axes, contact, and handle direction only. It does not draw an item mesh or unresolved pole and provides no sampled animation, solved IK, joint-limit result, clipping result, camera, capture, or immutable review packet. When those later surfaces land, humans and agents share a `reviewId` with explicit cameras; the shell must not scrape the World camera, cursor, or selection to invent that packet.
