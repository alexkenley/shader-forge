...
This document specifies the complete target contract. The current slice implements compatible skeleton/attachment parsing, deterministic rest/sample evaluation, native v2 two-bone IK, native cone-twist joint-limit diagnostics, prefab-bound authored visual-box evidence through `DataFoundation`, revision-safe transient sessiond queries, lease-gated attachment preview, and an Assets workbench that draws exact rest/sampled evaluator frames. Sessiond binds animation, authored content TOML, and the foundation manifest into one sorted revision snapshot and fails closed on drift or symbolic sources. The shell guards the complete manifest and draws only native eight-corner box evidence, explicitly labelling it non-collision and non-review. Schema-v1 two-hand profiles remain pre-IK. Runtime graph playback, serialized joint-limit diagnostics, sampled MCP evidence, clipping truth, native capture, and immutable review packets remain open.
...
- End-to-end serialization and presentation of native joint-limit results, plus numeric evaluation that transforms diagnostic skeleton capsules and the explicit prefab collision box into clipping results. The render-procgeo visual box cannot substitute for the collision component.
...
5. `DataFoundation` strictly parses an optional prefab-only box collision component into `PrefabSourceSnapshot` without inferring from render procgeo. Numeric joint/capsule/collision evaluation remains deferred.
...
A complete spatial-authoring workflow requires all gates below. The implemented slice satisfies parsing, validation, typed handles, deterministic v1/v2 cooking including authored joint-limit/capsule metadata, strict typed prefab box-collision source truth, native clip pose sampling, rest evaluation, sampled v2 two-bone IK with explicit v1 fallback, native numeric cone-twist diagnostics, prefab-bound authored visual-box evidence, exact transient rest/sample sessiond evidence, and non-review Assets rest/sampled schematics; serialized joint-limit presentation, numeric clipping diagnostics, sampled MCP evidence, rendered capture, and immutable review remain open.

1. Skeleton schema version 2 with hierarchy, roles, sockets, optional cone-twist limits, and optional diagnostic capsules validates natively. **Implemented. Native numeric cone-twist evaluation is implemented; serialization/presentation and capsule clipping remain open.**
...
8. Diagnostics are numeric and profile-driven. **Implemented natively for authored cone-twist limits in `diagnose` mode; unimplemented clamp policy is rejected. Serialized presentation and clipping remain open.**
...
12. Current three-bone metadata, proxy-card rendering, and non-review Assets schematics are described honestly wherever they remain. **Implemented for Assets: exact authored visual-box outlines, samples, and solved v2 IK frames are allowed, but the box is not a rendered mesh or collision truth and the native joint-limit result is not yet serialized to Assets. There is no available clipping result, camera, capture, or review-evidence claim.**
...
5. **Rest-pose schematic evaluation — implemented.** `shader_forge_spatial evaluate-rest` composes deterministic rest bone/socket, item, hand, joint-segment, target, and pole frames plus exact prefab-bound visual-box corners when available. A revision-safe sessiond GET evaluates exact animation/content/foundation bytes, preview returns transient exact baseline/candidate evaluations, and the shell projects resolved v2 poles and the exact box outline. All are explicitly unsampled, do not apply IK, and carry no rendered-mesh, collision, joint-limit, clipping, camera, capture, or review-packet claim.
