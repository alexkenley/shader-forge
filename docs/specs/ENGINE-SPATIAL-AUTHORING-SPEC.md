...
- Numeric evaluation that transforms the now-authored joint limits, diagnostic skeleton capsules, and explicit prefab collision box into joint-violation and clipping results. The render-procgeo visual box cannot substitute for the collision component.
...
1. **Schema and validator — implemented through authored diagnostic geometry.** V1 skeleton/attachment compatibility, strict v2 skeleton/socket/attachment parsing, optional cone-twist limits and diagnostic capsules, strict optional prefab box collision, item-space pole semantics, secondary-arm/palm capability checks, role and graph validation, quaternion canonicalization, and supported cross-references. Evaluator-side prefab/procgeo lookup is implemented through `DataFoundation`; numeric joint/capsule/collision evaluation remains deferred.
