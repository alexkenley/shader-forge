...
- The implemented native `capture-sample` path must be orchestrated by sessiond across every requested sample and required camera, including authored `player_camera` evidence where required. It depth-tests posed bone prisms and the authored procgeo item box into clean PNGs; it does not reuse runtime debug-proxy cards.
...
The native `capture-sample` command is a renderer primitive, not review publication. Spatial recapture remains fail-closed until sessiond revision/lease checks and atomic immutable packet publication exist. No caller may write proxy-card PNGs or standalone native frames and call them spatial reviews.
...
- sessiond orchestration of the implemented native close-camera renderer across revision-bound samples, plus required authored `player_camera` capture and atomic immutable packet publication
...
9. **Workbench and recapture — native close-camera renderer implemented.** `shader_forge_spatial capture-sample` evaluates one exact authored phase/time and software-rasterizes the authored procgeo item box plus every posed bone segment as depth-tested 3D triangles under fixed explicit lighting. It emits bounded camera/bounds/lighting metadata and four deterministic clean RGB PNGs only into a new output directory, with 50%-75% framing and cleanup on failure. It deliberately does not publish a review packet. Authored `player_camera` capture, multi-sample sessiond lease/revision orchestration, atomic immutable packets, and optional annotations remain deferred.
...
11. **`sf-mcp` adapter — attachment read, validation, and mutation implemented.** One session-pinned, lease-free `spatial_attachment_read` tool adapts the exact revision-bound rest/sample responses, while lease-free `spatial_attachment_validate` adapts operation validation with bounded strict samples and no credential. Process-owned identity and credentials plus explicit leases adapt preview/review/apply/undo. Capture, typed spatial resources, and recapture/review tools remain deferred.
