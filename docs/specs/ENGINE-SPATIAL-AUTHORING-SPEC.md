...
Status: native schema/query/cook/rest-schematic evaluation, compatible v1 plus strict v2 attachment profiles, deterministic schema-v2 clip sampling and sampled two-bone secondary-hand IK, authored v2 cone-twist parse/cook plus end-to-end diagnose-only joint-limit diagnostics, strict native/CLI and revision-safe transient sessiond rest/sample evaluation, semantic operation, constrained Assets tuner, and exact authored rest/sampled Assets schematics implemented; capsule/item clipping, collision evaluation, sampled MCP evidence, generic bake/runtime consumption, typed MCP review resources, and rendered review workflow deferred
...
This document specifies the complete target contract. The current slice implements compatible skeleton/attachment parsing, deterministic rest/sample evaluation, native v2 two-bone IK, authored v2 cone-twist parse/cook, native rest-relative swing and signed-twist joint-limit diagnostics serialized through `shader_forge_spatial`, fail-closed sessiond validation, diagnose-only Assets aggregate and per-bone PASS/FAIL, prefab-bound authored visual-box evidence through `DataFoundation`, revision-safe transient sessiond queries, lease-gated attachment preview, and an Assets workbench that draws exact rest/sampled evaluator frames. Sessiond binds animation, authored content TOML, and the foundation manifest into one sorted revision snapshot and fails closed on drift or symbolic sources. The shell guards the complete manifest and draws only native eight-corner box evidence, explicitly labelling it non-collision and non-review. Schema-v1 two-hand profiles remain pre-IK. Runtime graph playback, capsule/item clipping, collision evaluation, sampled MCP evidence, generic bake/runtime consumption, typed MCP review resources, native capture, and immutable review packets remain open.
...
- `shader_forge_spatial evaluate-rest --animation-root <path> --attachment <id> --content-root <path> --data-foundation <path>`, which composes the selected v2 skeleton rest pose, sockets, dominant/item frames, contact/handle frames, secondary target, versioned pole truth, and joint-segment endpoints into deterministic machine-readable geometry; it also resolves the referenced prefab through the existing `DataFoundation` and emits an exact eight-corner `authored_visual_box` when the prefab has one unambiguous valid box procgeo source, plus one exact typed `diagnostics.jointLimits` aggregate/per-bone JSON object
- `shader_forge_spatial evaluate-sample --animation-root <path> --attachment <id> --phase <phase> --normalized-time <value> --content-root <path> --data-foundation <path>`, which requires an exact authored envelope sample, composes the same geometry from the native clip sampler, applies primary attachment, runs deterministic two-bone secondary-hand IK for v2 two-hand profiles, and carries the same prefab-bound visual-box evidence and the same exact typed joint-limit object; v1 two-hand profiles remain explicitly pre-IK
- `GET /api/spatial/attachment/evaluate` and `/evaluate-sample`, which stage exact current authored animation bytes, every authored `content/**/*.toml` input, and `data/foundation/engine-data-layout.toml` through `SessionStore`; bind the selected source to the validator-selected attachment ID/schema/skeleton/item/mode/perspective contract; bind sample phase/time when present; validate the joint-limit object fail-closed against evaluator bones, order, roles, and recomputed swing/twist truth; and compare the complete staged revision manifest against the live tree before returning
...
The rest evaluator remains an unsolved schematic geometry query with `pose.sampled=false`. For v2 profiles it resolves the authored item-space pole into world space; v1 pole space remains unresolved. The sampled evaluator reports `pose.sampled=true`, applies primary attachment, and for v2 two-hand profiles applies `secondary_hand_ik` to the semantic upper-arm/lower-arm/hand chain using the secondary palm socket as the effector. Reachable targets align the palm frame to the target. Unreachable targets clamp only the physical wrist solve and report exact reach/contact/angular residual and tolerance truth. Schema-v1 two-hand profiles still report the IK layer unavailable and carry `pre_ik_only`; one-hand profiles carry `sampled_attachment_schematic_only`. Both evaluators expose the exact world-space corners of a prefab-bound box procgeo when available. That box is authored render-procgeo evidence only: it is not a rendered mesh, collision geometry, or clipping truth. Authored joint limits produce native numeric diagnostics that `shader_forge_spatial` serializes, sessiond validates fail-closed, and Assets presents as diagnose-only PASS/FAIL. Capsules and prefab collision remain source/loader truth only, so clipping remains unavailable. Every result remains non-review evidence. Sessiond exposes both reports transiently with complete fail-closed public validation and revision binding across animation, content, and the data-foundation manifest. Assets consumes both, draws the exact visual-box outline when available, and retains explicit non-review/no-collision labels. Candidate and MCP evidence remain rest-only. Sessiond evaluation reports never become operation validation or persisted review artifacts. Still deferred: collision transformation and numeric capsule/item overlap, generic `engine bake` integration, cooked-runtime loading, runtime graph consumption, blending, attachment rendering, sampled MCP evidence, native camera/capture, review packets, operation-scoped validation/recapture, native overlay tuning, and typed `sf-mcp` validation/review resources.
...
- Numeric evaluation that transforms diagnostic skeleton capsules and the explicit prefab collision box into clipping results. The render-procgeo visual box cannot substitute for the collision component. Authored cone-twist joint-limit parse/cook, native rest-relative evaluation, `shader_forge_spatial` serialization, sessiond fail-closed validation, and Assets diagnose-only PASS/FAIL are already implemented.
...
5. `DataFoundation` strictly parses an optional prefab-only box collision component into `PrefabSourceSnapshot` without inferring from render procgeo. Native rest/sample joint-limit evaluation is implemented; numeric capsule/item clipping and collision evaluation remain deferred.
...
8. The implemented rest evaluator composes parent-first rest bone transforms, sockets, attachment/item frames, hand targets, and joint-segment endpoints without running procedural IK. It resolves the attachment's `item_prefab` through `DataFoundation`, accepts only one valid box procgeo source, and emits eight deterministic world corners in the same order as the asset pipeline. It also emits one exact typed `diagnostics.jointLimits` object from rest-relative cone swing and signed twist without mutating or clamping the pose. V1 pole input remains unresolved; v2 item-space pole input is resolved. Reports fail closed on non-finite output and treat missing, ambiguous, invalid, or non-box visual geometry as typed unavailable evidence. Joint-limit unavailable is exactly `no_joint_limits_authored`.
9. The implemented sampled evaluator runs primary attachment and then deterministic two-bone secondary-hand IK for v2 two-hand profiles. It preserves limb lengths and stable skeleton order, aligns the palm socket to the target when reachable, clamps only the physical wrist solve when unreachable, and reports separate reach/contact/angular residuals and tolerances plus the same exact typed joint-limit object. Missing/misparented chains, zero-length limbs, non-finite math, and degenerate pole planes fail closed.
...
The schematic validates the complete public evaluation shape and bounded payload before rendering. Malformed nested values, non-finite vectors/quaternions/axes/transforms, contradictory IK booleans/numbers, joint-limit objects that fail evaluator bone/order/role or recomputed swing/twist checks, unsafe projection bounds, oversized text/arrays, excessive coordinate rows, unknown geometry reasons, non-positive box dimensions, or world corners that do not recompose from the item frame make the whole schematic unavailable. Front X/Y, Side Z/Y, and Top X/Z projections show evaluator bone segments/origins, sockets, item origin and orientation axes, contact, hands and palms, secondary target, resolved v2 pole, handle direction, and the exact twelve-edge visual-box outline when available. They never invent an item mesh, collision shape, or unresolved v1 pole. Exact box corners and diagnostics remain available outside the SVG. Joint limits show diagnose-only aggregate and per-bone PASS/FAIL, or the exact `no_joint_limits_authored` unavailable reason. The UI explicitly labels the box as authored render-procgeo evidence, not collision truth or review evidence.
...
- explicit `pose.sampled=false`, native available/unavailable joint-limit truth, unavailable item geometry/IK/clipping diagnostics, v1 unresolved versus v2 resolved item-space pole truth, and `not_review_evidence`
...
- Assets rest/sampled schematic deep validation across populated bone/segment/socket/item/contact/handle/hand/palm/target/pole/diagnostic/limitation entries, string/array/coordinate-row limits, contradictory IK truth, exact joint-limit aggregate/per-bone objects including `no_joint_limits_authored` and recomputed swing/twist truth, and overflow-safe projection guards
...
A complete spatial-authoring workflow requires all gates below. The implemented slice satisfies parsing, validation, typed handles, deterministic v1/v2 cooking including authored joint-limit/capsule metadata, strict typed prefab box-collision source truth, native clip pose sampling, rest evaluation, sampled v2 two-bone IK with explicit v1 fallback, native numeric cone-twist diagnostics serialized through `shader_forge_spatial`, fail-closed sessiond joint-limit validation, diagnose-only Assets aggregate/per-bone PASS/FAIL, prefab-bound authored visual-box evidence, exact transient rest/sample sessiond evidence, and non-review Assets rest/sampled schematics; numeric clipping diagnostics, sampled MCP evidence, rendered capture, and immutable review remain open.

1. Skeleton schema version 2 with hierarchy, roles, sockets, optional cone-twist limits, and optional diagnostic capsules validates natively. **Implemented. Native numeric cone-twist evaluation, `shader_forge_spatial` serialization, sessiond fail-closed validation, and Assets diagnose-only presentation are implemented; capsule clipping remains open.**
...
3. Prefabs are referenced by ID and do not contain copied grip fields. **Implemented for evaluator lookup and authored collision source: `item_prefab` resolves through `DataFoundation`, exact box procgeo corners remain visual-only, and an independent strict optional prefab collision box is typed snapshot truth. Collision evaluation remains open.**
...
8. Diagnostics are numeric and profile-driven. **Implemented natively and end to end for authored cone-twist limits in `diagnose` mode; unimplemented clamp policy is rejected. Clipping remains open.**
...
12. Current three-bone metadata, proxy-card rendering, and non-review Assets schematics are described honestly wherever they remain. **Implemented for Assets: exact authored visual-box outlines, samples, solved v2 IK frames, and diagnose-only joint-limit aggregate/per-bone PASS/FAIL are allowed, but the box is not a rendered mesh or collision truth. There is no available clipping result, camera, capture, or review-evidence claim.**
...
1. **Schema and validator — implemented through authored diagnostic geometry and native cone-twist evaluation.** V1 skeleton/attachment compatibility, strict v2 skeleton/socket/attachment parsing, optional cone-twist limits and diagnostic capsules, strict optional prefab box collision, item-space pole semantics, secondary-arm/palm capability checks, role and graph validation, quaternion canonicalization, supported cross-references, and non-mutating rest-relative swing/twist violation diagnostics are implemented. Evaluator-side prefab/procgeo lookup uses `DataFoundation`. Joint-limit serialization through `shader_forge_spatial`, sessiond fail-closed validation, and Assets diagnose-only PASS/FAIL are implemented; numeric capsule/collision evaluation remains deferred.
...
5. **Rest-pose schematic evaluation — implemented.** `shader_forge_spatial evaluate-rest` composes deterministic rest bone/socket, item, hand, joint-segment, target, and pole frames plus exact prefab-bound visual-box corners when available and one exact typed joint-limit object. A revision-safe sessiond GET evaluates exact animation/content/foundation bytes, validates joint-limit bones/order/roles and recomputed truth fail-closed, preview returns transient exact baseline/candidate evaluations, and the shell projects resolved v2 poles, the exact box outline, and diagnose-only joint-limit PASS/FAIL. All are explicitly unsampled, do not apply IK, and carry no rendered-mesh, collision, clipping, camera, capture, or review-packet claim.
...
8. **Sampling and procedural layers — native, sessiond, and Assets evidence implemented.** Native schema-v2 pose sampling feeds primary attachment and deterministic secondary-hand two-bone IK at exact authored envelope phase/time. Requested/applied/unavailable layers are explicit, v1 remains pre-IK, and v2 reports reachable/unreachable plus separate reach/contact/angular tolerance truth. Both rest and sampled reports carry the same exact typed joint-limit object. The transient sessiond sampled GET binds the complete input manifest and fail-closed joint-limit truth; Assets validates that manifest, exact requested sample, and diagnose-only joint-limit PASS/FAIL before display. Sampled MCP consumption remains deferred.
