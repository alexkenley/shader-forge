...
This document specifies the complete target contract. The current slice implements compatible skeleton/attachment parsing, deterministic rest/sample evaluation, native v2 two-bone IK, prefab-bound authored visual-box evidence through `DataFoundation`, revision-safe transient sessiond queries, lease-gated attachment preview, and an Assets workbench that draws exact rest/sampled evaluator frames. Sessiond binds animation, authored content TOML, and the foundation manifest into one sorted revision snapshot and fails closed on drift or symbolic sources. The shell guards the complete manifest and draws only native eight-corner box evidence, explicitly labelling it non-collision and non-review. Schema-v1 two-hand profiles remain pre-IK. Runtime graph playback, sampled MCP evidence, collision/joint/clipping truth, native capture, and immutable review packets remain open.
...
- Authored joint limits, diagnostic skeleton capsules, and explicit item collision geometry, so the implemented secondary-hand IK output can be checked for joint violations and clipping. The current prefab-bound visual box cannot satisfy this collision gate.
...
5. The evaluator resolves prefab render procgeo through `DataFoundation`; the animation loader still does not own or duplicate that catalogue. Explicit prefab collision geometry and numeric joint/capsule evaluation remain deferred.
...
A complete spatial-authoring workflow requires all gates below. The implemented slice satisfies parsing, validation, typed handles, deterministic v1/v2 cooking including authored joint-limit/capsule metadata, native clip pose sampling, rest evaluation, sampled v2 two-bone IK with explicit v1 fallback, prefab-bound authored visual-box evidence, exact transient rest/sample sessiond evidence, and non-review Assets rest/sampled schematics; explicit item collision, numeric joint/clipping diagnostics, sampled MCP evidence, rendered capture, and immutable review remain open.

1. Skeleton schema version 2 with hierarchy, roles, sockets, optional cone-twist limits, and optional diagnostic capsules validates natively. **Implemented.** Numeric limit/clipping evaluation remains open.
...
3. Prefabs are referenced by ID and do not contain copied grip fields. **Implemented for evaluator lookup: `item_prefab` resolves through `DataFoundation`, and exact box procgeo corners are exposed without a second geometry schema. Collision geometry remains open.**
...
1. **Schema and validator — implemented through skeleton diagnostic metadata.** V1 skeleton/attachment compatibility, strict v2 skeleton/socket/attachment parsing, optional cone-twist limits and diagnostic capsules, item-space pole semantics, secondary-arm/palm capability checks, role and graph validation, quaternion canonicalization, and supported cross-references. Evaluator-side prefab/procgeo lookup is implemented through `DataFoundation`; explicit prefab collision parsing and numeric joint/capsule evaluation remain deferred.
