...
- The `Workspace` right-panel tab now also exposes export-preset inspection, release-layout readiness, package generation with the default sibling ZIP archive, visible prep state, and last-package summary for the selected workspace.
...
- `engine export inspect [--root <path>] [--preset <id>] [--package-root <path>]` prints the resolved export preset, path readiness, cooked-asset counts, and last-package summary; `default` and `release` provide source-controlled Debug and Release desktop configurations.
