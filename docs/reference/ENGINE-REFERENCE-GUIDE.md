...
- Unity text-YAML scene conversion maps active `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs, preserves source file IDs, carries local position/quaternion rotation/scale into reviewable generated records, and maps enabled valid perspective `Camera` optics plus enabled valid non-trigger `BoxCollider` center/size geometry into strict prefab components. `m_IsActive: 0` objects and their hierarchy subtrees are omitted; disabled components and trigger colliders fail closed instead of becoming active target components.
