...
- Rest evaluation is a schematic query only: `pose.sampled=false`. Exact `authored_visual_box` corners may be available through the prefab render-procgeo chain, and Assets draws their twelve-edge outline. The box is not a rendered item mesh, collision truth, or review evidence; solved IK, joint-limit/clipping results, camera, and capture remain unavailable.
...
- `sf-mcp` now exposes the lease-gated attachment preview/review/apply/undo workflow. Prefab-bound visual geometry is implemented; explicit collision geometry, joint/capsule parsing, generic `engine bake` and runtime consumption, sampled MCP evidence, rendering/capture, review packets, and typed MCP validate/recapture/review tools remain deferred.
