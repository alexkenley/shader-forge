...
- Use `docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md` for the implemented native spatial schema/query/cook/rest and sampled attachment evaluation with pre-IK two-hand output, Assets rest-rig workbench, and attachment-preview operation slices plus the deferred IK/rendered review workflow.
...
- Use `Assets` for the constrained attachment-profile tuner and native-evaluated rest-rig schematic. Browsing/evaluation is lease-free; editing starts only after `Begin tuning` acquires the exact attachment lease.
...
- `GET /api/spatial/attachment/evaluate` reads an existing attachment at an exact SHA-256 revision, rejects symbolic paths, stages exact authored animation bytes, validates the source-to-profile mapping, evaluates that expected ID, and rechecks the live revision before returning. It requires no lease and creates no operation, journal entry, persisted evaluation, SSE event, or authored/cooked write.
...
- Preview returns transient `evaluation.baseline` and `evaluation.candidate` reports; a new file has a `null` baseline. Evaluator output must return the validator-selected attachment ID. These reports are absent from the durable operation journal and remain `pose.sampled=false`, so they are not review evidence.
...
- `engine build runtime` configures and builds the native runtime with CMake; `engine build spatial` builds the dependency-free native spatial validate/cook/evaluate-rest/evaluate-sample tool. Both resolve CMake from `SHADER_FORGE_CMAKE` first and then `cmake` on `PATH`.
...
- `engine spatial evaluate-rest --attachment <id> [--animation-root animation] [--build-dir build/runtime] [--config Debug]` prints deterministic machine-readable rest-pose bone/socket/hand/item frames and joint-segment endpoints. `pose.sampled=false`; item mesh geometry plus IK, joint-limit, and clipping diagnostics are unavailable; pole world space is unresolved; and the output is not rendered review evidence.
- `engine spatial evaluate-sample --attachment <id> --phase <phase> --normalized-time <value> [--animation-root animation] [--build-dir build/runtime] [--config Debug]` requires an exact authored envelope sample and prints deterministic sampled attachment geometry. Primary attachment is applied. Requested, applied, and unavailable procedural layers are explicit; two-hand output is `pre_ik_only`, one-hand output is a sampled attachment schematic, and neither is review evidence.
...
- The shell tracks runtime state, build state, bridge activity, and recent log tails in `Playtest`.
...
- Spatial authoring now has a native schema/query/cook/rest-schematic foundation, deterministic sampled attachment evaluation, semantic operations, strict CLI queries, and a constrained Assets primary-grip tuner with a native-evaluated rest-rig schematic. Native sampled queries are not yet sessiond/shell review evidence. See `docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md`.
...
- V1 skeleton/clip compatibility, strict v2 skeleton/socket/clip-track validation, v1 attachment-profile validation, sampleable motion-envelope cross-references, canonical quaternions, and generation-safe query handles now exist.
...
- `shader_forge_spatial validate` and `cook` reuse the exact `AnimationSystem` loader. `evaluate-rest` composes deterministic rest geometry. `evaluate-sample --attachment <id> --phase <phase> --normalized-time <value>` resolves an exact authored envelope sample and composes the same bone/socket/item/contact/hand geometry over the native clip sampler with explicit procedural-layer truth.
- Sessiond exposes a revision-safe transient GET for the current authored attachment and returns transient baseline/candidate evaluations from preview. Both paths stage exact bytes, bind evaluator output to validator-selected IDs, and persist no evaluation; preview additionally rechecks its lease after evaluator work.
- Assets displays authored evidence only for its exact session/path/revision and candidate evidence only for its exact operation/base/proposed/state. Editing values never moves cached evidence; it becomes visibly stale until reevaluated, and conflict stays stale even when numeric values match.
- The accessible responsive workbench offers Front X/Y, Side Z/Y, and Top X/Z projections, exact evaluator coordinates, diagnostics, path/revision identity, live/error text, and explicit direction-glyph limits. A complete bounded deep validator rejects malformed nested entries, oversized payloads, non-finite transforms, and unsafe projection bounds.
- Rest evaluation is a schematic query only: `pose.sampled=false`; item mesh geometry, solved IK, joint-limit and clipping results, camera, capture, and review evidence are unavailable; the v1 pole has `space="unresolved"` and no world transform, so it is not projected.
- Sampled evaluation is also read-only and not review evidence. For two-hand profiles it applies primary attachment, reports secondary-hand IK unavailable, and carries `pre_ik_only`; one-hand profiles report `sampled_attachment_schematic_only`. Sessiond, Assets, and `sf-mcp` still consume rest evidence only.
...
- `sf-mcp` now exposes the lease-gated attachment preview/review/apply/undo workflow. Prefab existence, joint/capsule parsing, generic `engine bake` and runtime-consumption integration, secondary-hand IK, sampled sessiond/shell/MCP evidence, rendering/capture, review packets, and typed MCP validate/recapture/review tools remain deferred.
...
- `npm run test:spatial-tool` requires a native compiler and runs the production validate/cook/evaluate-rest/evaluate-sample tool against isolated fixtures, checking deterministic rest/sample geometry, unchanged rest shape, pre-IK/one-hand truth labels, strict native/`engine` CLI behavior, and failure cases; WSL `g++` is required on Windows and `g++` elsewhere.
...
- `npm run test:spatial-shell` validates exact primary-grip source rewriting, strict unsupported-layout rejection with continued lease-free evaluation, complete evaluator-schema/size/projection guards, exact evidence binding, one shared SSE subscription and authoritative operation refetch, Assets-only mounting, explicit lease intent, operation-only routing, accessible/responsive schematic markers, unresolved-pole exclusion, and credential redaction markers.
...
- A native spatial schema/query/cook/rest and sampled attachment evaluation slice with pre-IK two-hand output, plus a constrained Assets tuner and accessible native-evaluated three-projection rest-rig schematic with exact evidence binding, guarded SSE reconciliation, and no sampled/rendered review claim in the shell.
...
- Spatial authoring still needs prefab-catalog and joint/capsule integration, generic bake/runtime consumption, secondary-hand IK, sampled sessiond/shell/MCP evidence, attachment rendering/capture, validate/recapture/review-packet operations, and their typed MCP tools beyond the implemented attachment mutation adapter.
