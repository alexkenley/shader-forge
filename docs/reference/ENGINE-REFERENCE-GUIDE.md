...
- `sf-mcp` exposes read-only `spatial_attachment_read` plus lease-free `spatial_attachment_validate`; the CLI exposes the same validation operation as `engine spatial validate-operation`. Both validation adapters accept the bounded exact sample schema, use the selected operation/session authority, and expose neither credentials, proposed bytes, nor raw native stderr. Lease-gated attachment preview/review/apply/undo remains the mutation adapter; runtime collision/physics use, rendering/capture, immutable review packets, and recapture/review tools remain deferred.
