...
- Spatial authoring now has a native schema/query/cook/rest-schematic foundation, deterministic sampled attachment evaluation, deterministic native close-camera sample rendering, semantic operations, strict CLI queries, revision-safe transient sessiond rest/sample evidence, and a constrained Assets primary-grip tuner with native-evaluated rest/sampled rig schematics. Sampled evidence and standalone native frames are not review packets. See `docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md`.
...
- `shader_forge_spatial capture-sample` uses that exact sampled pose to depth-test the authored procgeo item box and posed bone prisms into four deterministic clean RGB PNGs. It emits explicit close-camera, bounds, and lighting metadata, writes only to a new bounded output directory, and is a renderer primitive rather than immutable review publication.
...
- `sf-mcp` exposes read-only `spatial_attachment_read` plus lease-free `spatial_attachment_validate`; the CLI exposes the same validation operation as `engine spatial validate-operation`. Both validation adapters accept the bounded exact sample schema, use the selected operation/session authority, and expose neither credentials, proposed bytes, nor raw native stderr. Lease-gated attachment preview/review/apply/undo remains the mutation adapter; runtime collision/physics use, authored player-camera capture, immutable review packets, and recapture/review tools remain deferred.
...
- Spatial authoring still needs generic bake/runtime consumption, runtime collision/physics integration, attachment rendering and revision-bound native frame capture, recapture/review-packet operations, and typed review adapters beyond the implemented sessiond/CLI/MCP operation validation, revision-bound `spatial_attachment_read`, attachment mutation revalidation, diagnostics, and authored runtime camera.
