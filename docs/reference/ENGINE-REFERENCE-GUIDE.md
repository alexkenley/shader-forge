...
- The current migration slice now converts project structure plus Unity and Godot text-scene hierarchy/local transforms into a usable Shader Forge skeleton. Unity perspective Camera optics and BoxCollider geometry are mapped; prefab instances, other components, camera/collider enabled state, orthographic cameras, collider trigger/layer/material semantics, source assets, runtime physics consumption, and coordinate-system remediation remain manual alongside the broader parity gap.
