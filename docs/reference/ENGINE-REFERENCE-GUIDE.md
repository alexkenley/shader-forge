...
- `GET /api/spatial/attachment/evaluate` reads an existing attachment at an exact SHA-256 revision, rejects symbolic paths, stages exact authored animation bytes, validates the source-to-profile mapping, evaluates that expected ID, and rechecks every staged animation-source revision before returning. `GET /api/spatial/attachment/evaluate-sample` adds an exact authored phase/time and returns transient sampled pose/IK evidence under the same full-input revision binding. Neither route requires a lease or creates an operation, journal entry, persisted evaluation, SSE event, or authored/cooked write.
...
- Spatial authoring now has a native schema/query/cook/rest-schematic foundation, deterministic sampled attachment evaluation, semantic operations, strict CLI queries, revision-safe transient sessiond rest/sample evidence, and a constrained Assets primary-grip tuner with a native-evaluated rest-rig schematic. Sampled evidence is not yet exposed in the shell or promoted to review evidence. See `docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md`.
...
- Sessiond exposes revision-safe transient rest and sampled GETs for the current authored attachment and returns transient rest baseline/candidate evaluations from preview. These paths stage exact bytes, bind evaluator output to the validator-selected ID/schema/skeleton/item/mode/perspective contract, and persist no evaluation. Read-only evaluation returns the complete sorted source-revision manifest and rejects any staged animation-input drift; preview also rechecks that manifest and its lease after evaluator work.
...
- `sf-mcp` now exposes the lease-gated attachment preview/review/apply/undo workflow. Prefab existence, joint/capsule parsing, generic `engine bake` and runtime-consumption integration, sampled shell/MCP evidence, rendering/capture, review packets, and typed MCP validate/recapture/review tools remain deferred.
...
- Spatial authoring still needs prefab-catalog and joint/capsule integration, generic bake/runtime consumption, sampled shell/MCP evidence, attachment rendering/capture, validate/recapture/review-packet operations, and their typed MCP tools beyond the implemented attachment mutation adapter.
