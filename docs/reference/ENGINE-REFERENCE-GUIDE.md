...
- Current operation tools are bounded `operation_list`, `operation_read`, lease-free `spatial_attachment_validate`, `spatial_attachment_preview`, separate `operation_approve` / `operation_reject`, and spatial-only `operation_apply` / `operation_undo`.
- The MCP actor and session come from process state. Validation first rejects foreign-session operations and sends no credential or lease. Preview/apply/undo require the process-owned agent/credential and an owned granted write lease; apply/undo also require coverage for every persisted spatial resource key and sessiond rechecks immediately before mutation.
...
- `sf-mcp` exposes read-only `spatial_attachment_read` plus lease-free `spatial_attachment_validate`; the CLI exposes the same validation operation as `engine spatial validate-operation`. Both validation adapters accept the bounded exact sample schema, use the selected operation/session authority, and expose neither credentials, proposed bytes, nor raw native stderr. Lease-gated attachment preview/review/apply/undo remains the mutation adapter. Sessiond recapture and immutable reads are implemented; their thin CLI/MCP/shell adapters remain deferred with runtime collision/physics use.
...
- Spatial authoring still needs generic bake/runtime consumption, runtime collision/physics integration, attachment rendering, and typed CLI/MCP/shell review adapters beyond the implemented native close/player-camera capture, sessiond recapture and immutable review packets, sessiond/CLI/MCP operation validation, revision-bound `spatial_attachment_read`, attachment mutation revalidation, diagnostics, and authored runtime camera.
