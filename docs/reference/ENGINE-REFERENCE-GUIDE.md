...
- Godot text-scene conversion maps declared nodes into parent-linked entities, carries explicit position/rotation/scale fields into Shader Forge transforms, maps valid explicit perspective `Camera3D` optics, derives prefab tags from node types, retains source node/type comments, and reports mapped coverage; normalized node-name collisions are disambiguated deterministically.
...
- The current migration slice now converts project structure plus Unity and Godot text-scene hierarchy/local transforms and perspective camera optics into a usable Shader Forge skeleton. Unity BoxCollider geometry and MonoBehaviour binding provenance are also mapped; prefab instances, other components, component fields/behavior, camera/collider enabled state, orthographic cameras, collider trigger/layer/material semantics, source assets, runtime physics consumption, and coordinate-system remediation remain manual alongside the broader parity gap.
