...
- Use `docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md` for the implemented native spatial schema/query/cook/rest-schematic and attachment-preview operation slices plus the deferred visual tuning workflow.
...
- Use `Assets` to open the existing asset authoring tool directly.
...
- Spatial authoring now has a native schema/query/cook/rest-schematic foundation, semantic operations, CLI adapter, and constrained Assets primary-grip tuner. It is not yet a rendered spatial workbench. See `docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md`.
...
- Rest evaluation is a schematic query only: `pose.sampled=false`; item geometry, IK, joint-limit, and clipping diagnostics are unavailable; the v1 pole has `space="unresolved"` and no world transform; and the output is not a review packet or visual evidence.
...
- A native spatial schema/query/cook/rest-schematic slice with compatible v1 skeletons, strict v2 skeleton/socket and v1 attachment validation, generation-safe handles, isolated fixtures, deterministic validation, an atomic derived payload, and deterministic attachment frame/segment evaluation.
