...
- `sf-mcp` exposes read-only `spatial_attachment_read`, lease-free `spatial_attachment_validate`, explicit review reservation/recapture/read tools, and the typed review resource. The CLI exposes the same validation and review operations. These adapters use selected operation/session authority, explicit caller-owned leases, and expose neither credentials, proposed bytes, nor raw native stderr. Shell packet presentation remains deferred with runtime collision/physics use.
...
- Spatial authoring still needs generic bake/runtime consumption, runtime collision/physics integration, gameplay attachment rendering, and shell review presentation beyond the implemented native close/player-camera capture, sessiond recapture and immutable review packets, CLI/MCP review adapters, sessiond/CLI/MCP operation validation, revision-bound `spatial_attachment_read`, attachment mutation revalidation, diagnostics, and authored runtime camera.
