...
- Godot text-scene conversion maps declared nodes into parent-linked entities, carries explicit position/rotation/scale fields into Shader Forge transforms, maps valid explicit perspective `Camera3D` optics and enabled `CollisionShape3D`-referenced `BoxShape3D` geometry, derives prefab tags from node types, retains source node/type comments, and reports mapped coverage; `disabled = true` collision shapes fail closed and normalized node-name collisions are disambiguated deterministically.
