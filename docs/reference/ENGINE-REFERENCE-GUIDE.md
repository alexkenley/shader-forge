...
- `GET /api/ai/providers` and `POST /api/ai/test` expose deterministic fake, optional Ollama, and enabled OpenRouter provider inspection/smoke tests from `engine_sessiond`, without returning provider credentials. Collection `POST`/`GET /api/ai/jobs` plus per-job `GET`/`DELETE` provide a bounded process-scoped one-active-request queue with result-free listings, status, cancellation, provider abort propagation, and metadata-only `ai.job` lifecycle events on the existing SSE stream. Successful queued calls with provider-reported usage atomically persist cumulative per-workspace/provider token counts; `GET /api/ai/usage` and `engine ai usage` expose the credential-free summary.
