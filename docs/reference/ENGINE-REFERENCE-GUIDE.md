...
- Unity text-YAML scene conversion maps `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs, preserves source file IDs, carries local position/quaternion rotation/scale into reviewable generated records, and maps enabled valid perspective `Camera` optics plus enabled valid non-trigger `BoxCollider` center/size geometry into strict prefab components. Disabled instances and trigger colliders fail closed instead of becoming active target components.
