...
- The current migration slice now converts project structure plus Unity and Godot text-scene hierarchy/local transforms, perspective camera optics, and box-collision geometry into a usable Shader Forge skeleton. Unity MonoBehaviour binding provenance is also mapped; prefab instances, other components, component fields/behavior, camera/collider enabled state, orthographic cameras, physics-body/trigger/layer/material semantics, source assets, runtime physics consumption, and coordinate-system remediation remain manual alongside the broader parity gap.
