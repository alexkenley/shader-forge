...
- Use `docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md` for the implemented native spatial schema/query/cook/rest evaluation, v2 sampled two-bone secondary-hand IK, Assets rest-rig workbench, and attachment-preview operation slices plus the deferred sampled shell/rendered review workflow.
...
- `engine spatial evaluate-rest --attachment <id> [--animation-root animation] [--build-dir build/runtime] [--config Debug]` prints deterministic machine-readable rest-pose bone/socket/hand/item frames and joint-segment endpoints. `pose.sampled=false`; item mesh geometry plus solved IK, joint-limit, and clipping diagnostics are unavailable; v1 poles remain unresolved while v2 item-space poles resolve to world coordinates; and the output is not rendered review evidence.
- `engine spatial evaluate-sample --attachment <id> --phase <phase> --normalized-time <value> [--animation-root animation] [--build-dir build/runtime] [--config Debug]` requires an exact authored envelope sample and prints deterministic sampled attachment geometry. Primary attachment is applied. V2 two-hand profiles apply deterministic palm-effector IK with item-space pole and separate reach/contact/angular truth; v1 two-hand profiles remain `pre_ik_only`, one-hand output is a sampled attachment schematic, and none is review evidence.
...
- Rest evaluation is a schematic query only: `pose.sampled=false`; item mesh geometry, solved IK, joint-limit and clipping results, camera, capture, and review evidence are unavailable. V1 poles remain unresolved and unprojected; v2 item-space poles resolve to world coordinates and the Assets schematic projects them as green rings.
- Sampled evaluation is also read-only and not review evidence. V2 two-hand profiles apply deterministic two-bone secondary-hand IK using the palm socket and item-space pole, preserving limb lengths and reporting separate physical reach, contact, and angular residuals/tolerances. V1 two-hand profiles remain `pre_ik_only`; one-hand profiles report `sampled_attachment_schematic_only`. Sessiond, Assets, and `sf-mcp` still consume rest evidence only.
...
- `npm run test:spatial-operations` validates isolated full animation staging, validator/evaluator ID and schema binding, no-write native-backed attachment preview, durable context, lease contention and rename coverage, apply/undo lease renewal and revision checks, CORS, and temporary cleanup.
...
- `npm run test:spatial-shell` validates exact primary-grip source rewriting, strict unsupported-layout rejection with continued lease-free evaluation, complete v1/v2 evaluator-schema/size/projection guards, exact evidence binding, one shared SSE subscription and authoritative operation refetch, Assets-only mounting, explicit lease intent, operation-only routing, accessible/responsive schematic markers, unresolved-v1 exclusion, resolved-v2 pole projection, and credential redaction markers.
...
- A native spatial schema/query/cook/rest and sampled attachment evaluation slice with v2 two-bone secondary-hand IK and explicit v1 pre-IK compatibility, plus a constrained Assets tuner and accessible native-evaluated three-projection rest-rig schematic with resolved v2 pole projection, exact evidence binding, guarded SSE reconciliation, and no sampled/rendered review claim in the shell.
