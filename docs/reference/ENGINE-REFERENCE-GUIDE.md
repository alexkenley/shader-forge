...
- The current migration slice now converts project structure plus Unity and Godot text-scene hierarchy/local transforms into a usable Shader Forge skeleton. Unity perspective Camera optics are the first mapped serialized component; prefab instances, other components, orthographic/enabled-state semantics, source assets, and coordinate-system remediation remain manual alongside the broader parity gap.
