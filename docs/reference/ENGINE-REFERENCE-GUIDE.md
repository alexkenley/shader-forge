...
- A first CLI and shell/sessiond packaging lane that resolves export presets, emits release layouts under `build/package/`, records package reports plus launch manifests, and executes the default sibling ZIP hook.
...
- Packaging still needs runtime-build orchestration, installers, signing, richer preset families, additional platform hooks, and real cooked-runtime loading beyond the current release layout and default ZIP.
