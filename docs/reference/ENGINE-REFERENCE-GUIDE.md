...
- The current migration slice now converts project structure plus Unity and Godot text-scene hierarchy/local transforms, perspective camera optics, box-collision geometry, and exact script-binding provenance into a usable Shader Forge skeleton. Prefab instances, other components, component/node fields and behavior, camera/collider enabled state, orthographic cameras, physics-body/trigger/layer/material semantics, source assets, runtime physics consumption, and coordinate-system remediation remain manual alongside the broader parity gap.
