...
- Unity text-YAML scene conversion maps `GameObject` plus `Transform`/`RectTransform` documents into parent-linked entities and scene-owned prefabs, preserves source file IDs, carries local position/quaternion rotation/scale into reviewable generated records, and maps enabled valid perspective `Camera` optics plus enabled valid `BoxCollider` center/size geometry into strict prefab components. Disabled instances fail closed instead of becoming active target components.
...
- The current migration slice now converts project structure plus Unity and Godot text-scene hierarchy/local transforms, perspective camera optics, box-collision geometry, and exact script-binding provenance into a usable Shader Forge skeleton. Prefab instances, other components, component/node fields and behavior, disabled camera/collider preservation, orthographic cameras, physics-body/trigger/layer/material semantics, source assets, runtime physics consumption, and coordinate-system remediation remain manual alongside the broader parity gap.
