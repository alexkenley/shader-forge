...
- Rest evaluation is a schematic query only: `pose.sampled=false`. Exact `authored_visual_box` corners may be available through the prefab render-procgeo chain, and Assets draws their twelve-edge outline. The box is not a rendered item mesh, collision truth, or review evidence. Optional schema-v2 cone-twist limits and diagnostic capsules are typed authored/cooked truth, and optional prefab box collision is independent typed `DataFoundation` source truth; joint-limit/clipping results, camera, and capture remain unavailable.
...
- `sf-mcp` now exposes the lease-gated attachment preview/review/apply/undo workflow. Prefab-bound visual geometry, strict authored/cooked skeleton joint-limit/capsule metadata, and strict optional prefab box-collision source truth are implemented; numeric joint/clipping evaluation, generic `engine bake` and runtime consumption, sampled MCP evidence, rendering/capture, review packets, and typed MCP validate/recapture/review tools remain deferred.
...
- Spatial authoring still needs numeric joint-limit/capsule/prefab-collision evaluation, generic bake/runtime consumption, sampled MCP evidence, attachment rendering/capture, validate/recapture/review-packet operations, and their typed MCP tools beyond the implemented attachment mutation adapter.
