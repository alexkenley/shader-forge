...
- Use `docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md` for the implemented native spatial schema/query/cook/rest-schematic, Assets rest-rig workbench, and attachment-preview operation slices plus the deferred sampled/rendered review workflow.
...
- `engine build runtime` configures and builds the native runtime with CMake; `engine build spatial` builds the dependency-free native spatial validate/cook/evaluate-rest tool. Both resolve CMake from `SHADER_FORGE_CMAKE` first and then `cmake` on `PATH`.
...
- Spatial authoring now has a native schema/query/cook/rest-schematic foundation, semantic operations, CLI adapter, and a constrained Assets primary-grip tuner with a native-evaluated rest-rig schematic. It is not a sampled or rendered review workbench. See `docs/specs/ENGINE-SPATIAL-AUTHORING-SPEC.md`.
...
- `npm run test:spatial-tool` requires a native compiler and runs the production validate/cook/evaluate-rest tool against isolated fixtures, checking deterministic validation and cooking, exact rest geometry, non-commuting transform composition, canonical quaternion sign, unresolved pole space, non-finite failure, loader diagnostics, and CLI strict/help/build-first behavior; WSL `g++` is required on Windows and `g++` elsewhere.
...
- A native spatial schema/query/cook/rest-schematic slice plus a constrained Assets tuner and accessible native-evaluated three-projection rest-rig schematic with exact evidence binding, guarded SSE reconciliation, and no sampled/rendered review claim.
