...
- `engine spatial evaluate-rest --attachment <id> [--animation-root animation] [--content-root content] [--data-foundation data/foundation/engine-data-layout.toml] [--build-dir build/runtime] [--config Debug]` prints deterministic rest frames plus exact prefab-bound authored visual-box corners when one valid box procgeo resolves through `DataFoundation`. `pose.sampled=false`; the box is not a rendered mesh or collision shape, solved IK and joint/clipping diagnostics are unavailable, and the output is not review evidence.
- `engine spatial evaluate-sample --attachment <id> --phase <phase> --normalized-time <value> [--animation-root animation] [--content-root content] [--data-foundation data/foundation/engine-data-layout.toml] [--build-dir build/runtime] [--config Debug]` requires an exact authored envelope sample and carries the same prefab-bound visual-box evidence. V2 two-hand profiles apply deterministic palm-effector IK with separate reach/contact/angular truth; v1 remains `pre_ik_only`; none is collision or review evidence.
...
- Rest evaluation is a schematic query only: `pose.sampled=false`. Exact `authored_visual_box` corners may be available through the prefab render-procgeo chain, and Assets draws their twelve-edge outline. The box is not a rendered item mesh, collision truth, or review evidence. Authored v2 cone-twist metadata is parsed and cooked. Native rest/sample evaluation reports rest-relative swing and signed twist without mutating or clamping the pose. `shader_forge_spatial` emits one exact typed aggregate/per-bone `diagnostics.jointLimits` JSON object; sessiond validates it fail-closed against evaluator bones, order, roles, and recomputed truth; Assets shows diagnose-only aggregate and per-bone PASS/FAIL. Unavailable is exact `no_joint_limits_authored`. Clamp policy is rejected. Optional diagnostic capsules remain typed authored/cooked truth, and optional prefab box collision is independent typed `DataFoundation` source truth; clipping results, camera, and capture remain unavailable.
- Sampled evaluation is also read-only and not review evidence. V2 two-hand profiles apply deterministic two-bone secondary-hand IK using the palm socket and item-space pole, preserving limb lengths and reporting separate physical reach, contact, and angular residuals/tolerances. Both rest and sampled reports carry the same exact typed joint-limit object; Assets shows diagnose-only aggregate and per-bone PASS/FAIL, or exact `no_joint_limits_authored`. V1 two-hand profiles remain visibly `PRE-IK`; one-hand profiles report `sampled_attachment_schematic_only`. Sessiond and Assets expose this sampled evidence; candidate evaluation and `sf-mcp` remain rest-only.
...
- `sf-mcp` now exposes the lease-gated attachment preview/review/apply/undo workflow. Prefab-bound visual geometry, strict authored/cooked skeleton joint-limit/capsule metadata, native diagnose-only joint-limit evaluation serialized through `shader_forge_spatial` and fail-closed sessiond/Assets, and strict optional prefab box-collision source truth are implemented; capsule/item clipping, collision evaluation, generic `engine bake` and runtime consumption, sampled MCP evidence, rendering/capture, review packets, and typed MCP validate/recapture/review tools remain deferred.
...
- Spatial authoring still needs capsule/item clipping, collision evaluation, generic bake/runtime consumption, sampled MCP evidence, attachment rendering/capture, validate/recapture/review-packet operations, and their typed MCP tools beyond the implemented attachment mutation adapter and diagnose-only joint-limit slice.
