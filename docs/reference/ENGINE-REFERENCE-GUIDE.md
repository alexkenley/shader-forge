...
- Public operation list/detail/SSE views expose the bounded latest validation summary and event provenance but never private `beforeContent` or `proposedContent`. The HTTP/native evaluator orchestration that will produce these summaries is still deferred.
...
- The shell `World` workspace opens scene and prefab assets directly from the active session root and round-trips deterministic save, reload, revert, duplicate, and primary-object edits back to those files.
