...
- `npm run test:spatial-operations`: validate sessiond rest/sample attachment evidence, full-input revisions, validator-selected profile binding, preview, lease, durable context, apply, undo, and temporary staging.
...
- `npm run test:spatial-shell`: validate the Assets-only constrained primary-grip tuner, exact source rewrite, v1/v2 rest and sampled schematics, authored envelope/source-revision binding, explicit lock workflow, and operation-only route.
