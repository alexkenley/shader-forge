...
const buildStoreSource = fs.readFileSync(path.join(repoRoot, 'tools', 'engine-sessiond', 'lib', 'build-store.mjs'), 'utf8');
const cmakeCommandSourcePath = path.join(repoRoot, 'tools', 'shared', 'cmake-command.mjs');
const cmakeCommandSource = fs.readFileSync(cmakeCommandSourcePath, 'utf8');
const windowsNativeDepsScriptSource = fs.readFileSync(
  path.join(repoRoot, 'scripts', 'install-windows-native-runtime-deps.ps1'),
  'utf8',
);
const startDevCleanShellSource = fs.readFileSync(path.join(repoRoot, 'scripts', 'start-dev-clean.sh'), 'utf8');
const startDevCleanPowerShellSource = fs.readFileSync(path.join(repoRoot, 'scripts', 'start-dev-clean.ps1'), 'utf8');
...
assert.match(windowsNativeDepsScriptSource, /--recurse/);
assert.match(windowsNativeDepsScriptSource, /CMAKE_TOOLCHAIN_FILE/);
assert.match(windowsNativeDepsScriptSource, /Vulkan SDK/);
assert.match(startDevCleanShellSource, /SHADER_FORGE_CMAKE/);
assert.match(startDevCleanPowerShellSource, /SHADER_FORGE_CMAKE/);
assert.match(startDevCleanPowerShellSource, /Using vcpkg toolchain:/);
assert.match(startDevCleanPowerShellSource, /Using Vulkan SDK:/);
assert.match(startDevCleanShellSource, /Using CMake:/);
assert.match(startDevCleanPowerShellSource, /Using CMake:/);
assert.equal(cmakeCommandModule.normalizeConfiguredCommand('  "cmake"  '), 'cmake');
assert.equal(
  cmakeCommandModule.resolveCMakeCommand({ SHADER_FORGE_CMAKE: `"${process.execPath}"`, PATH: '' }),
  process.execPath,
);
assert.equal(
  cmakeCommandModule.resolveCMakeCommand({ SHADER_FORGE_CMAKE: '"/definitely/missing/cmake"', PATH: '' }),
  '',
);
assert.match(cmakeCommandModule.cmakeGuidanceMessage('runtime build'), /SHADER_FORGE_CMAKE/);
...
assert.match(dataFoundationHeader, /struct PrefabCameraComponentSnapshot/);
assert.match(dataFoundationHeader, /std::optional<PrefabCameraComponentSnapshot> cameraComponent/);
...
assert.match(saveHeader, /listSlots/);
...
assert.match(toolingHeader, /activeSaveSlotName/);
assert.match(toolingHeader, /saveSlotCount/);
...
assert.match(runtimeSource, /activeQuickSaveSlotName/);
assert.match(runtimeSource, /cycleRuntimeSaveSlot/);
assert.match(runtimeSource, /save_runtime_state/);
assert.match(runtimeSource, /load_runtime_state/);
assert.match(runtimeSource, /select_previous_save_slot/);
assert.match(runtimeSource, /select_next_save_slot/);
...
assert.match(runtimeSource, /kLegacyCameraVerticalFovDegrees = 70\.0F/);
assert.match(runtimeSource, /kLegacyCameraNearMeters = 0\.15F/);
assert.match(runtimeSource, /kLegacyCameraFarMeters = 1000\.0F/);
assert.match(runtimeSource, /cameraComponent->verticalFovDegrees/);
assert.match(runtimeSource, /source\.spawnTag == "player_camera" \? source\.cameraComponent : std::nullopt/);
assert.match(runtimeSource, /viewZ <= nearMeters \|\| viewZ > farMeters/);
assert.match(runtimeSource, /rollRadians = cameraRotation\[2\]/);
assert.match(runtimeSource, /rolledViewY/);
assert.match(runtimeSource, /is no longer compatible with the authored scene/);
assert.match(runtimeSource, /savedEntityIt->id != currentAuthorityId/);
...
assert.match(runtimeSource, /save=/);
assert.match(runtimeSource, /available_slots=/);
...
assert.match(runtimeSource, /built in stub mode/);
assert.match(runtimeSource, /SDL3 development files and the Vulkan SDK\/loader/);
...
assert.match(inputSource, /SDL_SCANCODE_F11/);
assert.match(inputSource, /SDL_SCANCODE_F12/);
...
assert.match(toolingSource, /save-slot=/);
assert.match(toolingSource, /save-slots=/);
