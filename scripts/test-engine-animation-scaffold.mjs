...
const spatialSkeletonToml = fs.readFileSync(path.join(spatialFixtureRoot, 'skeletons', 'spatial_humanoid.skeleton.toml'), 'utf8');
const attachmentToml = fs.readFileSync(path.join(spatialFixtureRoot, 'attachments', 'rifle_mk1_humanoid.attachment.toml'), 'utf8');
...
assert.match(animationHeader, /AttachmentProfileSnapshot/);
assert.match(animationHeader, /findAttachmentProfile/);
...
assert.match(spatialSkeletonToml, /schema_version = 2/);
assert.match(spatialSkeletonToml, /id = "humanoid\.standard\.v2"/);
assert.match(spatialSkeletonToml, /role = "hand_l"/);
assert.match(spatialSkeletonToml, /role = "hand_r"/);
assert.match(spatialSkeletonToml, /role = "primary_grip"/);
assert.match(attachmentToml, /id = "weapon\.rifle\.mk1\.humanoid"/);
assert.match(attachmentToml, /mode = "two_hand"/);
assert.match(attachmentToml, /\[secondary_hand\.target\]/);
