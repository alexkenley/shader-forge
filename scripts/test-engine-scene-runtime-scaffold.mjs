...
const cameraPrefabPath = path.join(repoRoot, 'content', 'prefabs', 'debug_camera.prefab.toml');
...
const cameraPrefab = fs.readFileSync(cameraPrefabPath, 'utf8');
...
assert.match(dataFoundationSource, /at most one prefab with spawn_tag 'player_camera'/);
assert.match(dataFoundationSource, /playerCameraEntity\.empty\(\) \? playerSpawnEntity : playerCameraEntity/);
...
assert.match(runtimeApp, /kLegacyCameraVerticalFovDegrees = 70\.0F/);
assert.match(runtimeApp, /kLegacyCameraNearMeters = 0\.15F/);
assert.match(runtimeApp, /kLegacyCameraFarMeters = 1000\.0F/);
assert.match(runtimeApp, /viewZ <= nearMeters \|\| viewZ > farMeters/);
...
assert.match(cameraPrefab, /\[component\.camera\]/);
assert.match(cameraPrefab, /projection = "perspective"/);
